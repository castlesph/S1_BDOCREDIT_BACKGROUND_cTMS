...
int inIITRead(int inSeekCnt)
{
	int result;
	int inResult = d_NO;
	char *sql = "SELECT inIssuerNumber, szIssuerAbbrev, szIssuerLabel, szPANFormat, szMaskBankCopy, szMaskMerchantCopy, szMaskCustomerCopy, szMaskExpireDate, szMaskDisplay, fMerchExpDate, fCustExpDate, fMerchPANFormat, inCheckHost, ulTransSeqCounter, fGetPolicyNumber, inPolicyNumMaxChar, szIssuerLogo, fLast4Digits, fQuasiCash, fGetPolicyNumber, fNSR, szNSRLimit, fNSRCustCopy, fBalInqAllowed, fMaskPanDisplay, szPrintOption, inIssuerID, fSMErrorRC86, fSMPrintFooterMsg, fEMVFallbackEnable, szCVMLimit FROM IIT WHERE IITid = ?";
...
{
	int result;
	char *sql = "UPDATE IIT SET szIssuerAbbrev = ?, szIssuerLabel = ?, szPANFormat = ?, szMaskBankCopy = ?, szMaskMerchantCopy = ?, szMaskCustomerCopy = ?, szMaskExpireDate = ?, szMaskDisplay = ?, fMerchExpDate = ?, fCustExpDate = ?, fMerchPANFormat = ?, ulTransSeqCounter = ?, fGetPolicyNumber = ?, inPolicyNumMaxChar = ?, szIssuerLogo = ?, fSMPrintFooterMsg = ?, fEMVFallbackEnable = ?, fMaskPanDisplay = ?, fNSR = ?, szNSRLimit = ?, fNSRCustCopy = ?, fLast4Digits = ?, fQuasiCash = ?, fBalInqAllowed = ? WHERE  IITid = ?";
	...
	//gcitra
	/*fPolicyNumber*/
	result = sqlite3_bind_int(stmt, inStmtSeq +=1, strIIT.fGetPolicyNumber);
	...
	/*fMaskPanDisplay*/
	result = sqlite3_bind_int(stmt, inStmtSeq +=1, strIIT.fMaskPanDisplay);

	/*fNSR*/
	result = sqlite3_bind_int(stmt, inStmtSeq +=1, strIIT.fNSR);

	/*szNSRLimit*/
	result = sqlite3_bind_text(stmt, inStmtSeq +=1, (char*)strIIT.szNSRLimit, strlen((char*)strIIT.szNSRLimit), SQLITE_STATIC);

	/*fNSRCustCopy*/
	result = sqlite3_bind_int(stmt, inStmtSeq +=1, strIIT.fNSRCustCopy);

	/*fLast4Digits*/
	result = sqlite3_bind_int(stmt, inStmtSeq +=1, strIIT.fLast4Digits);

	/*fQuasiCash*/
	result = sqlite3_bind_int(stmt, inStmtSeq +=1, strIIT.fQuasiCash);

	/*fBalInqAllowed*/
	result = sqlite3_bind_int(stmt, inStmtSeq +=1, strIIT.fBalInqAllowed);
...
int inMMTSave(int inSeekCnt)
{
	...
	vdDebug_LogPrintf("--inMMTSave--");
	vdDebug_LogPrintf("inSeekCnt=[%d]", inSeekCnt);
	vdDebug_LogPrintf("fBatchNotEmpty=[%d]", strMMT[0].fBatchNotEmpty);
	vdDebug_LogPrintf("fPendingReversal=[%d]", strMMT[0].fPendingReversal);
	vdDebug_LogPrintf("fPreAuthExisting=[%d]", strMMT[0].fPreAuthExisting);
	...
	/* prepare the sql, leave stmt ready for loop */
	result = sqlite3_prepare_v2(db, sql, -1, &stmt, NULL);
	vdDebug_LogPrintf("[inMMTSave]-sqlite3_prepare_v2[%d]",result);
	if (result != SQLITE_OK) {
		sqlite3_close(db);
		return 2;
	}
  ...
  //aaronnino for BDOCLG ver 9.0 fix on issue #00241 No Manual Settle/Clear Batch prompt after 3 failed end
	...
	vdDebug_LogPrintf("--inMMTSave--END");
...
int inTCTRead(int inSeekCnt)
{
  ...
  /*aaronnino for BDOCLG ver 9.0 fix on issue#00078 added inBankCopy, inCustomerCopy, inMerchantCopy*/
	...
	do {
		...
		/* szMinInstAmt */
		memcpy(strTCT.szMinInstAmt, sqlite3_column_blob(stmt,inStmtSeq +=1 ), 7);
...
//smac
   ...
   //aaronnino for BDOCLG ver 9.0 fix on issue #00078 Printing of copies should be configurable end 1 of 21
	   ...
	   //aaronnino for remote download setup 2 of 12 end
		...
		/*szMobileWalletLabel*/
		strcpy((char*)strTCT.szMobileWalletLabel, (char *)sqlite3_column_text(stmt,inStmtSeq +=1 ));

		/*inSetMobileWalletLabel*/
		strTCT.inSetMobileWalletLabel = sqlite3_column_int(stmt,inStmtSeq +=1 );
...
//smac 
  ...
  result = sqlite3_bind_int(stmt, inStmtSeq +=1, strTCT.fECRISOLogging);	
	...
	/*feReceiptEnable*/
	result = sqlite3_bind_int(stmt, inStmtSeq +=1, strTCT.feReceiptEnable);
	...
	/*fDCC*/
	result = sqlite3_bind_int(stmt, inStmtSeq +=1, strTCT.fDCC);

	/*fSignatureFlag*/
	result = sqlite3_bind_int(stmt, inStmtSeq +=1, strTCT.fSignatureFlag);

	/*fSMLogo*/
	result = sqlite3_bind_int(stmt, inStmtSeq +=1, strTCT.fSMLogo);

	/*szSMReceiptLogo*/
	result = sqlite3_bind_text(stmt, inStmtSeq +=1, (char*)strTCT.szSMReceiptLogo, strlen((char*)strTCT.szSMReceiptLogo), SQLITE_STATIC);

	/*szBDOReceiptLogo*/
	result = sqlite3_bind_text(stmt, inStmtSeq +=1, (char*)strTCT.szBDOReceiptLogo, strlen((char*)strTCT.szBDOReceiptLogo), SQLITE_STATIC);
...
int inDatabase_BatchInsert(TRANS_DATA_TABLE *transData)
{
	...
	/* Issue# 000096: BIN VER Checking -- jzg*/
 	char *sql = "INSERT INTO TransData (TransDataid, HDTid, MITid, CDTid, IITid, szHostLabel, szBatchNo, byTransType, byPanLen, szExpireDate, byEntryMode, szTotalAmount, szBaseAmount, szTipAmount, byOrgTransType, szMacBlock, szYear, szDate, szTime, szOrgDate, szOrgTime, szAuthCode, szRRN, szInvoiceNo, szOrgInvoiceNo, byPrintType, byVoided, byAdjusted, byUploaded, byTCuploaded, szCardholderName, szzAMEX4DBC, szStoreID, szRespCode, szServiceCode, byContinueTrans, byOffline, byReversal, byEMVFallBack, shTransResult, szTpdu, szIsoField03, szMassageType, szPAN, szCardLable, usTrack1Len, usTrack2Len, usTrack3Len, szTrack1Data, szTrack2Data, szTrack3Data, usChipDataLen, baChipData, usAdditionalDataLen, baAdditionalData, bWaveSID,usWaveSTransResult,bWaveSCVMAnalysis, ulTraceNum, ulOrgTraceNum, usTerminalCommunicationMode, ulSavedIndex, byPINEntryCapability, byPackType, szOrgAmount, szCVV2, inCardType, byTCFailUpCnt, byCardTypeNum, byEMVTransStatus, T5A_len, T5A, T5F2A, T5F30, T5F34, T5F34_len, T82, T84_len, T84, T8A, T91, T91Len, T95, T9A, T9C, T9F02, T9F03, T9F09, T9F10_len, T9F10, T9F1A, T9F26, T9F27, T9F33, T9F34, T9F35, T9F36_len, T9F36, T9F37, T9F41, T9F53, ISR, ISRLen, T9B, T5F24, T71Len, T71, T72Len, T72, T9F06, T9F1E, T9F28, T9F29, szChipLabel, szTID, szMID, szHostLabel, szBatchNo, inNumOfAdjust, fTIPAllow, fBINVer, szBINVerSTAN, szPolicyNumber, inSavedCurrencyIdx, fFooterLogo, fOnlineSALE, fCompletion, fVoidOffline, SmacPoints, SmacBalance, inSmacTender, fFleetCard, inFleetNumofLiters, szFleetProductCode, szFleetProductDesc, szSMACAmount, fECRTriggerTran, szPOSTID, szDCCCur, szDCCCurAmt, inDCCCurMU, szDCCFXRate, inDCCFXRateMU, szDCCFXRateRefID, szDCCCardType, szDCCFXMID, szDCCFXTID, szDCCMerchPOS, szDCCMarkupPer, inDCCMarkupPerMU, fDCC, szDCCCurSymbol, szDCCLocalAmount, szDCCLocalCur, szDCCLocalSymbol, fPrintSMCardHolder, fPrintCardHolderBal, fSMACFooter, fPrintExpFlag, byPreviouslyUploaded, szOrgRRN, szDCCLocalTipAmount, szSMACBDORewardsBalance, szLocalStoreID, fDualBrandedCredit, fDCCAuth, fBINRouteApproved, T9F6E, T9F6E_len, fCUPPINEntry) VALUES (NULL, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)";
...
int inDatabase_BatchSearch(TRANS_DATA_TABLE *transData, char *hexInvoiceNo)
{
	...
	/* Issue# 000096: BIN VER Checking -- jzg*/
	char *sql = "SELECT HDTid, MITid, CDTid, IITid, byTransType, byPanLen, szExpireDate, byEntryMode, szTotalAmount, szBaseAmount, szTipAmount, byOrgTransType, szMacBlock, szYear, szDate, szTime, szOrgDate, szOrgTime, szAuthCode, szRRN, szInvoiceNo, szOrgInvoiceNo, byPrintType, byVoided, byAdjusted, byUploaded, byTCuploaded, szCardholderName, szzAMEX4DBC, szStoreID, szRespCode, szServiceCode, byContinueTrans, byOffline, byReversal, byEMVFallBack, shTransResult, szTpdu, szIsoField03, szMassageType, szPAN, szCardLable, usTrack1Len, usTrack2Len, usTrack3Len, szTrack1Data, szTrack2Data, szTrack3Data, usChipDataLen, baChipData, usAdditionalDataLen, baAdditionalData, bWaveSID,usWaveSTransResult,bWaveSCVMAnalysis,ulTraceNum, ulOrgTraceNum, usTerminalCommunicationMode, ulSavedIndex, byPINEntryCapability, byPackType, szOrgAmount, szCVV2, inCardType, byTCFailUpCnt, byCardTypeNum, byEMVTransStatus, T5A_len, T5A, T5F2A, T5F30, T5F34, T5F34_len, T82, T84_len, T84, T8A, T91, T91Len, T95, T9A, T9C, T9F02, T9F03, T9F09, T9F10_len, T9F10, T9F1A, T9F26, T9F27, T9F33, T9F34, T9F35, T9F36_len, T9F36, T9F37, T9F41, T9F53, ISR, ISRLen, T9B, T5F24, T71Len, T71, T72Len, T72, T9F06, T9F1E, T9F28, T9F29, szChipLabel, szTID, szMID, szHostLabel, szBatchNo, inNumOfAdjust, fTIPAllow, fBINVer, szBINVerSTAN, szPolicyNumber, inSavedCurrencyIdx, fFooterLogo, fOnlineSALE, fCompletion, fVoidOffline, SmacPoints,SmacBalance, inSmacTender, fFleetCard, inFleetNumofLiters, szFleetProductCode, szFleetProductDesc, szSMACAmount, fECRTriggerTran, szPOSTID, szDCCCur, szDCCCurAmt, inDCCCurMU, szDCCFXRate, inDCCFXRateMU, szDCCFXRateRefID, szDCCCardType, szDCCFXMID, szDCCFXTID, szDCCMerchPOS, szDCCMarkupPer, inDCCMarkupPerMU, fDCC, szDCCCurSymbol, szDCCLocalAmount, szDCCLocalCur, szDCCLocalSymbol, fPrintSMCardHolder, fPrintCardHolderBal, fSMACFooter, fPrintExpFlag, byPreviouslyUploaded, szOrgRRN, szDCCLocalTipAmount, szSMACBDORewardsBalance, szLocalStoreID, fDualBrandedCredit, fDCCAuth, fBINRouteApproved, T9F6E, T9F6E_len, fCUPPINEntry FROM TransData WHERE szInvoiceNo = ?";
...
int inMultiAP_Database_BatchRead(TRANS_DATA_TABLE *transData)
{
	...
	result = sqlite3_exec( db, "begin", 0, 0, NULL );
	/* prepare the sql, leave stmt ready for loop */
	result = sqlite3_prepare_v2(db, sql, -1, &stmt, NULL);
	if (result != SQLITE_OK) {
		sqlite3_close(db);
...
int inMultiAP_Database_BatchRead(TRANS_DATA_TABLE *transData)
{
	int result;
	int inResult = d_NO;
	char *sql = "SELECT MITid, HDTid, CDTid, IITid, byTransType, byPanLen, szExpireDate, byEntryMode, szTotalAmount, szBaseAmount, szTipAmount, byOrgTransType, szMacBlock, szPINBlock, szYear, szDate, szTime, szOrgDate, szOrgTime, szAuthCode, szRRN, szInvoiceNo, szOrgInvoiceNo, byPrintType, byVoided, byAdjusted, byUploaded, byTCuploaded, szCardholderName, szzAMEX4DBC, szStoreID, szRespCode, szServiceCode, byContinueTrans, byOffline, byReversal, byEMVFallBack, shTransResult, szTpdu, szIsoField03, szMassageType, szPAN, szCardLable, usTrack1Len, usTrack2Len, usTrack3Len, szTrack1Data, szTrack2Data, szTrack3Data, usChipDataLen, baChipData, usAdditionalDataLen, baAdditionalData, bWaveSID,usWaveSTransResult,bWaveSCVMAnalysis, ulTraceNum, ulOrgTraceNum, usTerminalCommunicationMode, ulSavedIndex, byPINEntryCapability, byPackType, szOrgAmount, szCVV2, inCardType, byTCFailUpCnt, byCardTypeNum, byEMVTransStatus, T5A_len, T5A, T5F2A, T5F30, T5F34, T5F34_len, T82, T84_len, T84, T8A, T91, T91Len, T95, T9A, T9C, T9F02, T9F03, T9F09, T9F10_len, T9F10, T9F1A, T9F26, T9F27, T9F33, T9F34, T9F35, T9F36_len, T9F36, T9F37, T9F41, T9F53, ISR, ISRLen, T9B, T5F24, T71Len, T71, T72Len, T72, T9F06, T9F1E, T9F28, T9F29, szChipLabel, szTID, szMID, szHostLabel, szBatchNo, szECRPANFormatted, szECRRespText, szECRMerchantName, szECRRespCode, fBINVer, fECRTrxFlg FROM TransData WHERE TransDataid = ?";
...
{
	int result;
	char *sql = "UPDATE TransData SET HDTid = ?, MITid = ?, CDTid = ?, IITid = ?, szHostLabel = ?, szBatchNo = ?, byTransType = ?, byPanLen = ?, szExpireDate = ?, byEntryMode = ?, szTotalAmount = ?, szBaseAmount = ?, szTipAmount = ?, byOrgTransType = ?, szMacBlock = ?, szYear = ?, szDate = ?, szTime = ?, szOrgDate = ?, szOrgTime = ?, szAuthCode = ?, szRRN = ?, szInvoiceNo = ?, szOrgInvoiceNo = ?, byPrintType = ?, byVoided = ?, byAdjusted = ?, byUploaded = ?, byTCuploaded = ?, szCardholderName = ?, szzAMEX4DBC = ?, szStoreID = ?, szRespCode = ?, szServiceCode = ?, byContinueTrans = ?, byOffline = ?, byReversal = ?, byEMVFallBack = ?, shTransResult = ?, szTpdu = ?, szIsoField03 = ?, szMassageType = ?, szPAN = ?, szCardLable = ?, usTrack1Len = ?, usTrack2Len = ?, usTrack3Len = ?, szTrack1Data = ?, szTrack2Data = ?, szTrack3Data = ?, usChipDataLen = ?, baChipData = ?, usAdditionalDataLen = ?, baAdditionalData = ?, bWaveSID = ?,usWaveSTransResult = ?,bWaveSCVMAnalysis = ?, ulTraceNum = ?, ulOrgTraceNum = ?, usTerminalCommunicationMode = ?, ulSavedIndex = ?, byPINEntryCapability = ?, byPackType = ?, szOrgAmount = ?, szCVV2 = ?, inCardType = ?, byTCFailUpCnt = ?, byCardTypeNum = ?, byEMVTransStatus = ?, T5A_len = ?, T5A = ?, T5F2A = ?, T5F30 = ?, T5F34 = ?, T5F34_len = ?, T82 = ?, T84_len = ?, T84 = ?, T8A = ?, T91 = ?, T91Len = ?, T95 = ?, T9A = ?, T9C = ?, T9F02 = ?, T9F03 = ?, T9F09 = ?, T9F10_len = ?, T9F10 = ?, T9F1A = ?, T9F26 = ?, T9F27 = ?, T9F33 = ?, T9F34 = ?, T9F35 = ?, T9F36_len = ?, T9F36 = ?, T9F37 = ?, T9F41 = ?, T9F53 = ?, ISR = ?, ISRLen = ?, T9B = ?, T5F24 = ?, T71Len = ?, T71 = ?, T72Len = ?, T72 = ?, T9F06 = ?, T9F1E = ?, T9F28 = ?, T9F29 = ?, szChipLabel = ?, szTID = ?, szMID = ?, fBINVer=?, szBINVerSTAN=? szECRPANFormatted=? szECRRespText=?, szECRMerchantName=?  szECRRespCode=?, fECRTrxFlg = ?, T9F6E = ?, T9F6E_len = ?, szResponseText = ? WHERE TransDataid = ?";
...
int inMultiAP_Database_BatchInsert(TRANS_DATA_TABLE *transData)
{
	int result;	
	char *sql1 = "SELECT MAX(TransDataid) FROM TransData";
 	char *sql = "INSERT INTO TransData (TransDataid, HDTid, MITid, CDTid, IITid, szHostLabel, szBatchNo, byTransType, byPanLen, szExpireDate, byEntryMode, szTotalAmount, szBaseAmount, szTipAmount, byOrgTransType, szMacBlock, szYear, szDate, szTime, szOrgDate, szOrgTime, szAuthCode, szRRN, szInvoiceNo, szOrgInvoiceNo, byPrintType, byVoided, byAdjusted, byUploaded, byTCuploaded, szCardholderName, szzAMEX4DBC, szStoreID, szRespCode, szServiceCode, byContinueTrans, byOffline, byReversal, byEMVFallBack, shTransResult, szTpdu, szIsoField03, szMassageType, szPAN, szCardLable, usTrack1Len, usTrack2Len, usTrack3Len, szTrack1Data, szTrack2Data, szTrack3Data, usChipDataLen, baChipData, usAdditionalDataLen, baAdditionalData, bWaveSID,usWaveSTransResult,bWaveSCVMAnalysis, ulTraceNum, ulOrgTraceNum, usTerminalCommunicationMode, ulSavedIndex, byPINEntryCapability, byPackType, szOrgAmount, szCVV2, inCardType, byTCFailUpCnt, byCardTypeNum, byEMVTransStatus, T5A_len, T5A, T5F2A, T5F30, T5F34, T5F34_len, T82, T84_len, T84, T8A, T91, T91Len, T95, T9A, T9C, T9F02, T9F03, T9F09, T9F10_len, T9F10, T9F1A, T9F26, T9F27, T9F33, T9F34, T9F35, T9F36_len, T9F36, T9F37, T9F41, T9F53, ISR, ISRLen, T9B, T5F24, T71Len, T71, T72Len, T72, T9F06, T9F1E, T9F28, T9F29, szChipLabel, szTID, szMID, fBINVer, szBINVerSTAN, szECRPANFormatted, szECRRespText, szECRMerchantName, szECRRespCode, PRMid, szPolicyNumber, fECRTrxFlg, T9F6E, T9F6E_len, szResponseText) VALUES (1, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)";
...
int inBatchReviewNumRecord(void)
{
	int len;
	int result;
	char *sql = "SELECT COUNT(*) FROM TransData WHERE HDTid = ? AND MITid = ? AND szBatchNo = ? AND byTransType != ? AND byTransType != ? AND byTransType != ?";
	int inCount = 0;
...
int inReadDynamicMenu(int inIndex)
{
	...
	{
		case 1:
			sprintf(szTempSQL, "SELECT szButtonItemLabel, szButtonItemFunc, szSubMenuName FROM DMTrxMenu WHERE fButtonIDEnable > 0 ORDER BY usButtonID");
			break;
		case 2:
			sprintf(szTempSQL, "SELECT szButtonItemLabel, szButtonItemFunc, szSubMenuName FROM DMTerminalSettingMenu WHERE fButtonIDEnable > 0 ORDER BY usButtonID");
			break;
		case 3: // Quick menu
			sprintf(szTempSQL, "SELECT szButtonItemLabel, szButtonItemFunc, szSubMenuName FROM DMTrxMenu WHERE fButtonIDEnable > 0 ORDER BY usButtonID");
			break;
	...
	{
	 	case 3: // Quick menu
	 		strcpy(szMenuFilename, "/data/data/com.Source.S1_BDO.BDO/QUICKMENU.S3DB");
			break;
		default:
			if (strTCT.inMenuid > 99)
				sprintf(szMenuFilename, "/data/data/com.Source.S1_BDO.BDO/DYNAMICMENU%03d.S3DB", strTCT.inMenuid);
			else
				sprintf(szMenuFilename, "/data/data/com.Source.S1_BDO.BDO/DYNAMICMENU%02d.S3DB", strTCT.inMenuid);
			break;
	}
...
{
	int result;
  	char *sql = "SELECT TransDataid FROM TransData WHERE HDTid = ? AND MITid = ? AND szBatchNo = ? AND byTransType != ? AND byTransType != ? AND byTransType != ?";
	int count = 0;
...
{    
	...
	BOOL isExist = FALSE;
	char szFileName[d_BUFF_SIZE] = {0};

	vdDebug_LogPrintf("--inMMTReadHostName--");
	vdDebug_LogPrintf("inMITid=[%d]", inMITid);
	...
	do {
		...
		if (result == SQLITE_ROW) { /* can read data */
			...
			sprintf(szFileName, "%s%02d%02d.rev"
                                ...
                                , inCPTID[inCount]
                                , inMITid);
			isExist = isReversalFileExist(szFileName);
			
			vdDebug_LogPrintf("With indicator->HDTid=[%d],HostName=[%s],fBatchNotEmpty=[%d],fPendingReversal=[%d],fReprintSettleStatus=[%d],szFileName=[%s],isExist=[%d]",inCPTID[inCount],szHostName[inCount],fBatchNotEmpty,fPendingReversal,fReprintSettleStatus,szFileName,isExist);

			if (inIndicator == 1){			
				if (fBatchNotEmpty == TRUE)
					strcat((char*)szHostName[inCount],"*");
			}else if(inIndicator == 2) {
				if (fPendingReversal == TRUE || isExist == TRUE)
					strcat((char*)szHostName[inCount],"*");
...
int inMMTNumRecordwithBatch(int inMITid, int inHDTid[])
{
	...
	vdDebug_LogPrintf("--inMMTNumRecordwithBatch--");
	vdDebug_LogPrintf("inMITid=[%d]", inMITid);
...
int inDatabase_inSMACFooterRead(char *szBuff, TRANS_DATA_TABLE *transData)
{
	...
	do {
		...
		if (result == SQLITE_ROW) { /* can read data */
			...
			vdDebug_LogPrintf("REINER :: szBuff[%s]",szBuff);
		}
	} while (result == SQLITE_ROW);
...
/*
	vdDebug_LogPrintf("saturn --inDatabase_TerminalCloseDatabase--");
	sqlite3_exec(db,"commit;",NULL,NULL,NULL);
	sqlite3_finalize(stmt);
	sqlite3_close(db);
	vdDebug_LogPrintf("close db");
*/	
...
{
	int result;
	char *sql = "UPDATE IIT SET szIssuerAbbrev = ?, szIssuerLabel = ?, szPANFormat = ?, szMaskBankCopy = ?, szMaskMerchantCopy = ?, szMaskCustomerCopy = ?, szMaskExpireDate = ?, szMaskDisplay = ?, fMerchExpDate = ?, fCustExpDate = ?, fMerchPANFormat = ?, ulTransSeqCounter = ?, fGetPolicyNumber = ?, inPolicyNumMaxChar = ?, szIssuerLogo = ?, fSMPrintFooterMsg = ?, fEMVFallbackEnable = ? WHERE  IITid = ?";
...
int inFLGRead(int inSeekCnt)
{
	...
	char *sql = "SELECT fLargeInvoice, fDCCCTLS, fNewDCCMode, fQRECRMenu, fATPCUPMANUAL, fGrabpayRetieveLast FROM FLG WHERE FLGid = ?";
	...
	do {
		...
		if (result == SQLITE_ROW) { /* can read data */
			...
			strFLG.fDCCCTLS = fGetBoolean((BYTE *)sqlite3_column_text(stmt,inStmtSeq +=1 ));
			strFLG.fNewDCCMode = fGetBoolean((BYTE *)sqlite3_column_text(stmt,inStmtSeq +=1 ));
			strFLG.fQRECRMenu = fGetBoolean((BYTE *)sqlite3_column_text(stmt,inStmtSeq +=1 ));
			strFLG.fATPCUPMANUAL = fGetBoolean((BYTE *)sqlite3_column_text(stmt,inStmtSeq +=1 ));
			strFLG.fGrabpayRetieveLast = fGetBoolean((BYTE *)sqlite3_column_text(stmt,inStmtSeq +=1 ));
...
int inReadDMTrx(int inMenu)
{
	int inMenuIndex = 0,
			...
			inCtr = 0;
	char szMenuFilename[FILENAME_SIZE] = {0};
	char *szSQL_credit = "SELECT szButtonItemLabel, szButtonItemFunc, szSubMenuName FROM DMTrxMenu WHERE fButtonIDEnable = 1 ORDER BY usButtonID";
	char *szSQL_install = "SELECT szButtonItemLabel, szButtonItemFunc, szSubMenuName FROM DMInstallmentMenu WHERE fButtonIDEnable = 1 ORDER BY usButtonID";
	char *szSQL_hotel = "SELECT szButtonItemLabel, szButtonItemFunc, szSubMenuName FROM DMHotelMenu WHERE fButtonIDEnable = 1 ORDER BY usButtonID";
	char *szSQL_reprint = "SELECT szButtonItemLabel, szButtonItemFunc, szSubMenuName FROM DMReprintMenu WHERE fButtonIDEnable = 1 ORDER BY usButtonID";
	char *szSQL_report = "SELECT szButtonItemLabel, szButtonItemFunc, szSubMenuName FROM DMReportMenu WHERE fButtonIDEnable = 1 ORDER BY usButtonID";
	char *szSQL_setup = "SELECT szButtonItemLabel, szButtonItemFunc, szSubMenuName FROM DMTerminalSettingMenu WHERE fButtonIDEnable = 1 ORDER BY usButtonID";
	char *szSQL_detail = "SELECT szButtonItemLabel, szButtonItemFunc, szSubMenuName FROM DMDetailReportMenu WHERE fButtonIDEnable = 1 ORDER BY usButtonID";
	char *szSQL_smac = "SELECT szButtonItemLabel, szButtonItemFunc, szSubMenuName FROM DMSmacMenu WHERE fButtonIDEnable = 1 ORDER BY usButtonID";

	vdDebug_LogPrintf("inReadDMTrx, inMenu=[%d]", inMenu);
	...
	{
		case 1:
			inResult = sqlite3_prepare_v2(db, szSQL_credit  , -1, &stmt, NULL);
			break;
		case 2:
			inResult = sqlite3_prepare_v2(db, szSQL_install  , -1, &stmt, NULL);
			break;
		case 3:
			inResult = sqlite3_prepare_v2(db, szSQL_hotel, -1, &stmt, NULL);
			break;
		case 4:
			inResult = sqlite3_prepare_v2(db, szSQL_reprint, -1, &stmt, NULL);
			break;
		case 5:
			inResult = sqlite3_prepare_v2(db, szSQL_report, -1, &stmt, NULL);
			break;
		case 6:
			inResult = sqlite3_prepare_v2(db, szSQL_setup, -1, &stmt, NULL);
			break;
		case 7:
			inResult = sqlite3_prepare_v2(db, szSQL_detail, -1, &stmt, NULL);
			break;
		case 8:
			inResult = sqlite3_prepare_v2(db, szSQL_smac, -1, &stmt, NULL);
			break;	
...
int inTMSEXRead(int inSeekCnt)
{
	int result;
	int inResult = d_NO;
	char *sql = "SELECT TMSEXid, szTMSID, szSerialNo, szGroupName, szTMSHostIP, inTMSHostPortNum, szUserName, szPassword, szTLSHostIP, inTLSHostPortNum, szTMSMode FROM TMSEX WHERE TMSEXid = ?";
	...
	do {
		...
		if (result == SQLITE_ROW) { /* can read data */
			...
			/*TMSEXid*/
			strTMSEX.TMSEXid = sqlite3_column_int(stmt, inStmtSeq);
			
			/*szTMSID*/
			strcpy((char*)strTMSEX.szTMSID, (char *)sqlite3_column_text(stmt,inStmtSeq +=1));
	...
	vdDebug_LogPrintf("strTMSEX.szTMSHostIP=[%s],Port=[%d]", strTMSEX.szTMSHostIP, strTMSEX.inTMSHostPortNum);
	vdDebug_LogPrintf("strTMSEX.szTLSHostIP=[%s],Port=[%d]", strTMSEX.szTLSHostIP, strTMSEX.inTLSHostPortNum);
...
{
	int result;
	char *sql = "UPDATE TMSEX SET szSerialNo = ?, szTMSMode = ?, szTMSHostIP = ?, szTLSHostIP = ?, inTMSHostPortNum = ?, inTLSHostPortNum = ? WHERE  TMSEXid = ?";
	...
	result = sqlite3_bind_text(stmt, inStmtSeq +=1, (char*)strTMSEX.szTMSMode, strlen((char*)strTMSEX.szTMSMode), SQLITE_STATIC);
	result = sqlite3_bind_text(stmt, inStmtSeq +=1, (char*)strTMSEX.szTMSHostIP, strlen((char*)strTMSEX.szTMSHostIP), SQLITE_STATIC);
	result = sqlite3_bind_text(stmt, inStmtSeq +=1, (char*)strTMSEX.szTLSHostIP, strlen((char*)strTMSEX.szTLSHostIP), SQLITE_STATIC);
	result = sqlite3_bind_int(stmt, inStmtSeq +=1, strTMSEX.inTMSHostPortNum);
	result = sqlite3_bind_int(stmt, inStmtSeq +=1, strTMSEX.inTLSHostPortNum);
...
int inUpdateCommsMode(int inCommMode, int inIPHeader)
{
	...
	//sqlite3_bind_int(stmt, inStmtSeq +=1, inIPHeader);
    result = sqlite3_bind_int(stmt, inStmtSeq +=1, 1);
...
int inPHQRRead(int inSeekCnt)
{
	...
	int result;
	int inResult = d_NO;
	char *sql = "SELECT MerchantID, ScanQRURL, GenerateQRURL, GetStatusURL, VoidURL, inCurlRequestTimeout, inCurlRetries, SSLEnable, szPAN, szFailureMsg1, szFailureMsg2, szFailureMsg3, szProxyURL, szAppKey, szAppSecret, szMerchantAPIUser, szMerchantAPIKey, fSignatureLine, fDisclaimer, fThankYouLine, fPrintTimeLine, fPartnerCode, szTranStatusAppkey, szTranStatusSecret, szGetStatusPriIP, szGetStatusSecIP, inGetStatusPriPort, inGetStatusSecPort FROM PHT WHERE PHTID = ?";

	vdDebug_LogPrintf("inPHQRRead");
	vdDebug_LogPrintf("inSeekCnt=[%d]", inSeekCnt);

	/* open the database */
	result = sqlite3_open(DB_BDOPAY,&db);
	...
	result = sqlite3_bind_int(stmt, 1, inSeekCnt);
	vdDebug_LogPrintf("sqlite3_bind_int, result=[%d]", result);
	...
	do {
		...
		if (result == SQLITE_ROW) { /* can read data */
			...
			strcpy((char*)strPHQR.MerchantID, (char *)sqlite3_column_text(stmt, inStmtSeq));
			
			strcpy((char*)strPHQR.ScanQRURL, (char *)sqlite3_column_text(stmt, inStmtSeq +=1));
			
			strcpy((char*)strPHQR.GenerateQRURL, (char *)sqlite3_column_text(stmt, inStmtSeq +=1));	

			strcpy((char*)strPHQR.GetStatusURL, (char *)sqlite3_column_text(stmt, inStmtSeq +=1));	

			strcpy((char*)strPHQR.VoidURL, (char *)sqlite3_column_text(stmt, inStmtSeq +=1));	
			...
			strPHQR.SSLEnable = sqlite3_column_int(stmt,inStmtSeq +=1 );

			strcpy(strPHQR.szPAN, sqlite3_column_text(stmt,inStmtSeq +=1));

			strcpy(strPHQR.szFailureMsg1, sqlite3_column_text(stmt,inStmtSeq +=1));

			strcpy(strPHQR.szFailureMsg2, sqlite3_column_text(stmt,inStmtSeq +=1));

			strcpy(strPHQR.szFailureMsg3, sqlite3_column_text(stmt,inStmtSeq +=1));			

			strcpy(strPHQR.szProxyURL, sqlite3_column_text(stmt,inStmtSeq +=1));
			
			strcpy(strPHQR.szAppKey, sqlite3_column_text(stmt,inStmtSeq +=1));

			strcpy(strPHQR.szAppSecret, sqlite3_column_text(stmt,inStmtSeq +=1));

			strcpy(strPHQR.szMerchantAPIUser, sqlite3_column_text(stmt,inStmtSeq +=1));

			strcpy(strPHQR.szMerchantAPIKey, sqlite3_column_text(stmt,inStmtSeq +=1));
			
			strPHQR.fSignatureLine = sqlite3_column_int(stmt,inStmtSeq +=1 );
			
			strPHQR.fDisclaimer = sqlite3_column_int(stmt,inStmtSeq +=1 );
			
			strPHQR.fThankYouLine = sqlite3_column_int(stmt,inStmtSeq +=1 );
			
			strPHQR.fPrintTimeLine = sqlite3_column_int(stmt,inStmtSeq +=1 );
			
			strPHQR.fPartnerCode = sqlite3_column_int(stmt,inStmtSeq +=1 );
			
			strcpy(strPHQR.szTranStatusAppkey, sqlite3_column_text(stmt,inStmtSeq +=1));

			strcpy(strPHQR.szTranStatusSecret, sqlite3_column_text(stmt,inStmtSeq +=1));

			strcpy((char*)strPHQR.szGetStatusPriIP, (char *)sqlite3_column_text(stmt,inStmtSeq +=1 ));
			
			strcpy((char*)strPHQR.szGetStatusSecIP, (char *)sqlite3_column_text(stmt,inStmtSeq +=1 ));
			
			strPHQR.inGetStatusPriPort = sqlite3_column_int(stmt, inStmtSeq +=1 );
			
			strPHQR.inGetStatusSecPort = sqlite3_column_int(stmt, inStmtSeq +=1 );
	...
	vdDebug_LogPrintf("strPHQR.szGetStatusPriIP=[%s]", strPHQR.szGetStatusPriIP);
	vdDebug_LogPrintf("strPHQR.szGetStatusSecIP=[%s]", strPHQR.szGetStatusSecIP);
	vdDebug_LogPrintf("strPHQR.inGetStatusPriPort=[%ld]", strPHQR.inGetStatusPriPort);
	vdDebug_LogPrintf("strPHQR.inGetStatusSecPort=[%ld]", strPHQR.inGetStatusSecPort);

	vdDebug_LogPrintf("return inPHQRRead");
    return(inResult);
}

// Update ip only -- sidumili
int inCPT_IPUpdate(char *szPriTxnHostIP, char *szSecTxnHostIP, char *szPriSettlementHostIP, char *szSecSettlementHostIP)
{
	int result;
    char *sql = "UPDATE CPT SET szPriTxnHostIP = ?, szSecTxnHostIP = ?, szPriSettlementHostIP = ?, szSecSettlementHostIP = ? WHERE fIpUpdate = 1";
...
{
	int result;
    char *sql = "UPDATE CPT SET inPriTxnHostPortNum = ?, inSecTxnHostPortNum = ?, inPriSettlementHostPort = ?, inSecSettlementHostPort = ? WHERE fPortUpdate = 1";
...
{    
	...
	vdDebug_LogPrintf("--inMMTReadPreAuthHostName--");
	...
	do {
		...
		if (result == SQLITE_ROW) { /* can read data */
			...
			vdDebug_LogPrintf("HDTid=[%d]", inCPTID[inCount]);
			vdDebug_LogPrintf("fBatchNotEmpty=[%d]", fBatchNotEmpty);
			vdDebug_LogPrintf("fPendingReversal=[%d]", fPendingReversal);
			vdDebug_LogPrintf("fReprintSettleStatus=[%d]", fReprintSettleStatus);
			vdDebug_LogPrintf("fPreAuthEnable=[%d]", fPreAuthEnable);
			vdDebug_LogPrintf("fPreAuthExisting=[%d]", fPreAuthExisting);
...
{
	int result;
	char *sql = "UPDATE HDT SET inHostIndex = ?, szTPDU = ?, szNII = ? ,fHostEnable = ? ,szTraceNo = ? , fPrintBankReceipt = ?, fPrintCustReceipt = ?, fPrintMercReceipt = ?, fPrintVoidBankReceipt = ?, fPrintVoidCustReceipt = ?, fPrintVoidMercReceipt = ?, szDetailReportMask = ? WHERE  HDTid = ?";

	vdDebug_LogPrintf("--inHDTUpdate--");
	vdDebug_LogPrintf("inSeekCnt=[%d]", inSeekCnt);
	/* open the database */
	result = sqlite3_open(DB_TERMINAL,&db);
	if (result != SQLITE_OK) {
	...
	result = sqlite3_bind_text(stmt, inStmtSeq +=1, (char*)strHDT.szDetailReportMask, strlen((char*)strHDT.szDetailReportMask), SQLITE_STATIC);
...
{    
	int result;	
	char *sql = "SELECT IITid, szIssuerLabel FROM IIT ORDER BY szIssuerLabel";
	int inCount = 0;
	int inDBResult = 0;
...
int inSetMobileWalletLabel(char *szButtonItemLabel)
{
	...
	char *sql = "UPDATE DMTrxMenu SET szButtonItemLabel=? WHERE szButtonItemFunc='inQRPAY'"; 			
...
{    
	int result;	
	char *sql = "SELECT szTransactionTypeName, szImageFileName FROM IMG WHERE fTxnEnable = ?";
	int inCount = 0;
	int inDBResult = 0;
	BYTE szTemp[110];

	vdDebug_LogPrintf("--inIMGRead--");
	...
	do {
		...
		if (result == SQLITE_ROW) { /* can read data */
			...
			strcpy((char*)szTemp, (char *)sqlite3_column_text(stmt,inStmtSeq ));
			//vdDebug_LogPrintf("strImageList[%d].szTransactionTypeName[%s]",inCount, strImageList[inCount].szTransactionTypeName);

			// szTranTitle
			strcpy((char*)strImageList[inCount].szTranTitle, szTemp);

			// szTransactionTypeName
			SetToUpperCase((char*)szTemp);
			strcpy((char*)strImageList[inCount].szTransactionTypeName, szTemp);
			
			/*szImageFileName*/
			strcpy((char*)strImageList[inCount].szImageFileName, (char *)sqlite3_column_text(stmt,inStmtSeq +=1 ));
			//vdDebug_LogPrintf("strImageList[%d].szImageFileName[%s]",inCount, strImageList[inCount].szImageFileName);
			
			// Debug
			vdDebug_LogPrintf("inCount=[%d],szTranTitle=[%s],szTransactionTypeName=[%s],szImageFileName=[%s]",inCount, strImageList[inCount].szTranTitle, strImageList[inCount].szTransactionTypeName, strImageList[inCount].szImageFileName);
...
{
	int result;
    char *sql = "UPDATE PHT SET szGetStatusPriIP = ?, szGetStatusSecIP = ? WHERE PHTID = ?";

	vdDebug_LogPrintf("--inPHQRIPUpdate--");
	vdDebug_LogPrintf("inSeekCnt=[%d]", inSeekCnt);
	vdDebug_LogPrintf("szPriTxnHostIP=[%s]", szPriTxnHostIP);
	vdDebug_LogPrintf("szSecTxnHostIP=[%s]", szSecTxnHostIP);
...
{
	int result;
    char *sql = "UPDATE PHT SET inGetStatusPriPort = ?, inGetStatusSecPort = ? WHERE PHTID = ?";

	vdDebug_LogPrintf("--inPHQRPortUpdate--");
	vdDebug_LogPrintf("inSeekCnt=[%d]", inSeekCnt);
	vdDebug_LogPrintf("inPriTxnHostPortNum=[%ld]", inPriTxnHostPortNum);
	vdDebug_LogPrintf("inSecTxnHostPortNum=[%ld]", inSecTxnHostPortNum);
...
int inBDOPayRead(int inSeekCnt)
{
	...
	int result;
	int inResult = d_NO;
	char *sql = "SELECT MerchantID, szProxyURL, inProxyPort FROM BPT WHERE BPTID = ?";

	vdDebug_LogPrintf("--inBDOPayRead--");
	vdDebug_LogPrintf("inSeekCnt=[%d]", inSeekCnt);
	...
	do {
		...
		if (result == SQLITE_ROW) { /* can read data */
			...
			strcpy((char*)strBDOPAY.MerchantID, (char *)sqlite3_column_text(stmt, inStmtSeq));
			
			strcpy(strBDOPAY.szProxyURL, sqlite3_column_text(stmt,inStmtSeq +=1));
			
			strBDOPAY.inProxyPort= sqlite3_column_int(stmt,inStmtSeq +=1 );
	...
	vdDebug_LogPrintf("strBDOPAY.szProxyURL=[%s]", strBDOPAY.szProxyURL);
	vdDebug_LogPrintf("strBDOPAY.inProxyPort=[%ld]", strBDOPAY.inProxyPort);
...
{
	int result;
    char *sql = "UPDATE BPT SET szProxyURL = ? WHERE BPTID = ?";

	vdDebug_LogPrintf("--inBDOPayIPUpdate--");
	vdDebug_LogPrintf("inSeekCnt=[%d]", inSeekCnt);
	vdDebug_LogPrintf("szPriTxnHostIP=[%s]", szPriTxnHostIP);
...
int inGetMenuID(int inSeekCnt)
{
	...
	char *sql = "SELECT inMainLine, fHandsetPresent, fDemo, inMenuid, fECR, inMinBattery FROM TCT WHERE TCTid = ?";

	vdDebug_LogPrintf("--inGetMenuID--");
	vdDebug_LogPrintf("inSeekCnt=[%d]", inSeekCnt);
...
int inGetDynamicMenuDetail(char* szButtonItemLabel)
{
	...
	vdDebug_LogPrintf("--inGetDynamicMenuDetail--");
	...
	sprintf(szTempSQL, "SELECT szButtonItemLabel, szButtonItemFunc, szSubMenuName FROM DMTrxMenu WHERE fButtonIDEnable > 0 AND szButtonItemLabel = '%s' ORDER BY usButtonID", szButtonItemLabel);
	...
	do 
	{
		...
		{ 
			...
			/* szButtonTitleLabel */			
			strcpy((char*)strGBLVar.szGBLButtonItemLabel, (char *)sqlite3_column_text(stmt, inStmtSeq));

			/* szButtonItemFunc */			
			strcpy((char*)strGBLVar.szGBLButtonItemFunc, (char *)sqlite3_column_text(stmt, inStmtSeq += 1));

			/* szSubMenuName */			
			strcpy((char*)strGBLVar.szGBLSubMenuName, (char *)sqlite3_column_text(stmt, inStmtSeq += 1));
	...
	vdDebug_LogPrintf("strGBLVar.szGBLButtonItemLabel=[%s]", strGBLVar.szGBLButtonItemLabel);
	vdDebug_LogPrintf("strGBLVar.szGBLButtonItemFunc=[%s]", strGBLVar.szGBLButtonItemFunc);
	vdDebug_LogPrintf("strGBLVar.szGBLSubMenuName=[%s]", strGBLVar.szGBLSubMenuName);
...
char* getHostLabel(int inSeekCnt, char* szHostLabel)
{
	...
	int result;
	int inResult = d_NO;
	char *sql = "SELECT inHostIndex, szHostLabel FROM HDT WHERE HDTid = ?";
	int HDTid = 0;
	...
	do {
		...
		if (result == SQLITE_ROW) { /* can read data */
            ...
            /*inHostIndex*/
			HDTid = sqlite3_column_int(stmt, inStmtSeq );

            /* szHostLabel */
			strcpy((char*)szHostLabel, (char *)sqlite3_column_text(stmt, inStmtSeq +=1));
...
int getHostIndex(char* szHostLabel)
{
	...
	vdDebug_LogPrintf("--getHostIndex--");
	vdDebug_LogPrintf("szHostLabel=[%s]",szHostLabel);

	memset(szTempSQL, 0, sizeof(szTempSQL));
	sql=&szTempSQL;

	sprintf(szTempSQL, "SELECT inHostIndex, szHostLabel FROM HDT WHERE upper(szHostLabel) = '%s'", szHostLabel);
	vdDebug_LogPrintf("szTempSQL=[%s]", szTempSQL);
...
{
	int result;	
	char *sql = "SELECT szTransactionTypeName, szImageFileName FROM IMG WHERE inTxnTypeID = ? AND fTxnEnable = ? ORDER BY inSequence";
	int inCount = 0;
	int inDBResult = 0;
	BYTE szTemp[110];

	vdDebug_LogPrintf("--inReadIMGMenu--");
	vdDebug_LogPrintf("pTxnTpe=[%d]",pTxnTpe);
	...
	inStmtSeq = 0;
	sqlite3_bind_int(stmt, inStmtSeq +=1, pTxnTpe);
	sqlite3_bind_int(stmt, inStmtSeq +=1, 1); // fTxnEnable
	...
	do 
	{
		...
		{ 
			...
			// Debug
			vdDebug_LogPrintf("inCount=[%d],szTransactionTypeName=[%s],szImageFileName=[%s]",inCount, strImageList[inCount].szTransactionTypeName, strImageList[inCount].szImageFileName);
...
char* getImageFileName(char* szHostLabel, char* szFileName)
{
	...
	vdDebug_LogPrintf("--getImageFileName--");
	vdDebug_LogPrintf("szHostLabel=[%s]",szHostLabel);

	memset(szTempSQL, 0, sizeof(szTempSQL));
	sql=&szTempSQL;

	sprintf(szTempSQL, "SELECT IMGid, szImageFileName FROM IMG WHERE upper(szTransactionTypeName) = '%s'", szHostLabel);
	vdDebug_LogPrintf("szTempSQL=[%s]", szTempSQL);
	...
	do {
		...
		if (result == SQLITE_ROW) { /* can read data */
            ...
            /*inHostIndex*/
			IMGid = sqlite3_column_int(stmt, inStmtSeq );

            /* szHostLabel */
			strcpy((char*)szFileName, (char *)sqlite3_column_text(stmt, inStmtSeq +=1));           
		}
	} while (result == SQLITE_ROW);
...
int getCTLSMode(int inSeekCnt)
{
	...
	char *sql = "SELECT inMainLine, byCtlsMode FROM TCT WHERE TCTid = ?";
