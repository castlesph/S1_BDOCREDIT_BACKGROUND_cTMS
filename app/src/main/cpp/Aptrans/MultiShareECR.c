...
USHORT usCTOSS_ECRInitialize(void)
{
    ...
    //usResult = inMultiAP_RunIPCCmdTypes("com.Source.SHARLS_ECR.SHARLS_ECR", d_IPC_CMD_ECR_Initialize, bInBuf, usInLen, bOutBuf, &usOutLen);
...
USHORT usCTOSS_ECR_MultiDataSet(IN USHORT usLen, IN BYTE *pValue)
{
    ...
    memset(bOutBuf, 0x00, sizeof(bOutBuf));    
    //usResult = inMultiAP_RunIPCCmdTypes("com.Source.SHARLS_ECR.SHARLS_ECR", d_IPC_CMD_ECR_SetPackageData, bInBuf, usInLen, bOutBuf, &usOutLen);
