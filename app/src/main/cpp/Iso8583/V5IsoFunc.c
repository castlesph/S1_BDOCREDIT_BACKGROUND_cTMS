...
int inBuildAndSendIsoData(void)
{
	...
	vdDebug_LogPrintf("--inBuildAndSendIsoData--");
	vdDebug_LogPrintf("srTransRec.HDTid=[%d]", srTransRec.HDTid);

	inEFTPubRead(srTransRec.HDTid);
	vdDebug_LogPrintf("strEFTPub.inEFTEnable=[%d]", strEFTPub.inEFTEnable);
	if (strEFTPub.inEFTEnable > 0)
	{
		inRet = inCheckKeys(EFTSEC_KEYSET, EFTSEC_KEYINDEX);
		vdDebug_LogPrintf("SATURN check keys return %d", inRet);
		if (inRet != d_OK)
			return d_NO;
	}
	
	vdDisplayMessageBox(1, 8, "Processing", (srTransRec.byTransType == SETTLE ? "settlement" : "transaction"), MSG_PLS_WAIT, MSG_TYPE_PROCESS);
	CTOS_Delay(10);
  ...
  srTransPara = &srTransRec;
    ...
    //usCTOSS_LCDDisplay(szDisMsg);
		...
		//vdDispTransTitleCardTypeandTotal(srTransRec.byTransType, "PROCESSING...");
	#endif
	//vdDisplayMessage("PROCESSING...", "", "");
    ...
    {        
					...
					{            
						...
						if (srTransPara->shTransResult == TRANS_TERMINATE)
						{
						...
						else  
							{
								...
								else if (srTransRec.usTerminalCommunicationMode == GPRS_MODE)
								{
        ...
        if(srTransPara->byTransType == SETTLE)
        {
			...
			if ((inResult = inProcessEMVTCUpload_PreAuthDCC(srTransPara, -1)) != ST_SUCCESS)
			{
				...
				strcpy(szDisMsg, szTitle);
				strcat(szDisMsg, "|");
				strcat(szDisMsg, "TC Upload Error");
				//usCTOSS_LCDDisplay(szDisMsg);
				inDisplayMessageBoxWithButtonConfirmation(1,8,"","TC Upload Error","","",MSG_TYPE_INFO, BUTTON_TYPE_NONE_OK);
				CTOS_Beep();
				return ST_ERROR;
			}
			else							
    ...
    {	  
        if (CN_FALSE == srTransPara->byOffline)
        {
            ...
            if ((inResult = inBuildOnlineMsg(srTransPara)) != ST_SUCCESS)
            {			
				vdDebug_LogPrintf("inBuildOnlineMsg(srTransPara) = [%d]", inResult);
				if (((srTransPara->byEntryMode == CARD_ENTRY_ICC) ||
				//(srTransPara->bWaveSID == d_VW_SID_JCB_WAVE_QVSDC) ||		
				...
				vdDebug_LogPrintf("inBuildOnlineMsg, byTransType=[%d]",srTransPara->byTransType);
				if(srTransPara->byTransType != SETTLE){
					...
					else
					{
						if(srTransPara->byPackType != SEND_ADVICE){
							...
							{
								vdDebug_LogPrintf("Connection Terminated. Please Try Again");
            ...
            if((srTransPara->byTransType == SETTLE) && (!memcmp(srTransPara->szRespCode,"95",2)))
            {
                ...
                if(inResult != ST_SUCCESS)
                {
									...
									else
									{
										...
										//usCTOSS_LCDDisplay(szDisMsg);
										inDisplayMessageBoxWithButtonConfirmation(1,8,"","BATCH UPLOAD ERROR","","",MSG_TYPE_INFO, BUTTON_TYPE_NONE_OK);
										CTOS_Beep();
										CTOS_Delay(1500);
									}
...
int inSnedReversalToHost(TRANS_DATA_TABLE *srTransPara, int inTransCode, short shTxnFlag)
{
	...
	vdDebug_LogPrintf("shTxnFlag=[%d]", shTxnFlag);
	vdDebug_LogPrintf("byEMVReversal=[%d]", srTransPara->byEMVReversal);
	vdDebug_LogPrintf("szRRN=[%s]", srTransPara->szRRN);
	vdDebug_LogPrintf("szAuthCode=[%s]", srTransPara->szAuthCode);

    if((inResult = inMyFile_CheckFileExist(szFileName)) < 0)
    {
        vdDebug_LogPrintf("SATURN inMyFile_CheckFileExist <0");
        return ST_SUCCESS;
    }
    
    inSendLen = inResult;
    
    vdDebug_LogPrintf("SATURN inMyFile_ReversalRead(%d)",inResult);
    ...
    {    
		...
		if (shTxnFlag == POST_TXN)//only post_txn need reset the field39
		{
			...
			{
				vdDebug_LogPrintf("SATURN modify reversal");
		...
		vdDebug_LogPrintf("fModifyReversal=[%d]", fModifyReversal);
		if(fModifyReversal == TRUE)
        {
            ...
            {
                vdDebug_LogPrintf("SATURN . inSave_inMyFile_ReversalSave(%04x)",inResult);
                inCTOS_inDisconnect();
                inResult = ST_ERROR;
            }	
        }		
        ...
        {
            vdDebug_LogPrintf("SATURN inSnedReversalToHost Send Err %d", inReceLen);
            srTransRec.shTransResult = TRANS_COMM_ERROR;
        ...
        srTransPara->ulTraceNum = wub_bcd_2_long(strHDT.szTraceNo, 3);			
		srTransPara->ulOrgTraceNum = srTransPara->ulTraceNum;//Update ulOrgTraceNum for application not to use STAN of reversal on TC Upload.
        /*albert - end - 20140718 - modified reversal stan, increment*/	 
        ...
        {   
			   ...
			   (memcmp(srTransPara->szRespCode, "95", 2) == 0 && srTransPara->HDTid == SMSHOPCARD_HDT_INDEX ) )
    		{
                ...
                if (inResult != d_OK)
    			{
    			    vdDebug_LogPrintf("SATURN . inSnedReversalToHost %04x",inResult);
                    inCTOS_inDisconnect();
                    return ST_ERROR;
    			} 
                else{
                    ...
                    vdDebug_LogPrintf("SATURN rev. file deelted succesfully after send rev to host");
    		...
    		{
    		    vdDebug_LogPrintf("SATURN. inSnedReversalToHost Resp Err %02x%02x",srTransPara->szRespCode[0],srTransPara->szRespCode[1]);
                inCTOS_inDisconnect();
    ...
    vdDebug_LogPrintf("SATURN **inSnedReversalToHost END**");
...
{    
	...
	vdDebug_LogPrintf("TINE:  inProcessReversal -- 1 --for CODE TRACING ONLY....");
	if ((strlen(srTransPara->szRespCode) > 0) && (inResult == ST_SUCCESS))
	{
        ...
        vdDebug_LogPrintf("(strlen(srTransPara->szRespCode) > 0) && (inResult == ST_SUCCESS)[%d]", srTransRec.szRespCode);
...
void vdModifyBitMapFunc(int inTransCode, int *inBitMap)
{
	...
	vdDebug_LogPrintf("saturn modifybitmap");
...
//smac
	...
	vdDebug_LogPrintf("szRRN=[%s]", srTransRec.szRRN);
	vdDebug_LogPrintf("szAuthCode=[%s]", srTransRec.szAuthCode);
...
//EMV: Online PIN enchancement - end -- jzg
	...
	if (srTransPara->HDTid == CUP_HDT_INDEX || srTransPara->HDTid == CUPUSD_HDT_INDEX)
	{
		vdDebug_LogPrintf("Host is CUP...");
		...
		if (srTransRec.byEntryMode == CARD_ENTRY_ICC){
			...
			if(srTransRec.fEMVPIN == TRUE)
			{  
				if(inTransCode != REVERSAL && inTransCode != TC_UPLOAD)
			   	{
					vdCTOSS_SetBitMapOn(inBitMap, 52);
					vdCTOSS_SetBitMapOn(inBitMap, 53);
				}
			}
			else if (srTransRec.fEMVPIN == FALSE)
			{
				vdCTOSS_SetBitMapOff(inBitMap, 52);
				vdCTOSS_SetBitMapOff(inBitMap, 53);
			}		
		}
		else
		{
		   
			if (srTransPara->byTransType != VOID)// DE 52 is included on the data on Void w/ PIN transactions
			{
				//if(srTransRec.fCUPPINEntry == TRUE) 
				if (srTransRec.fEMVPINEntered == TRUE)// for emv certification
			   	{
			   	...
			   	//else if (srTransRec.fCUPPINEntry == FALSE) 
			   	if (srTransRec.fEMVPINEntered == FALSE)// for emv certificatio
			   	{
				   vdCTOSS_SetBitMapOff(inBitMap, 52);
				   vdCTOSS_SetBitMapOff(inBitMap, 53);
			   	}
		...
		/* BDOCLG-00115: add DE55 to reversal - start -- jzg */
		if((inTransCode == REVERSAL)&&(srTransPara->byEntryMode == CARD_ENTRY_ICC || srTransPara->byEntryMode == CARD_ENTRY_WAVE))
		{
			vdCTOSS_SetBitMapOn(inBitMap, 55);
		...
		/* BDOCLG-00115: add DE55 to reversal - end -- jzg */
	
		/* BDOCLG-00116: remove DE55 from void transaction - start -- jzg */
		if((inTransCode == VOID || inTransCode == VOID_REVERSAL)&&(srTransPara->byEntryMode == CARD_ENTRY_ICC || srTransPara->byEntryMode == CARD_ENTRY_WAVE))
		{
			vdCTOSS_SetBitMapOff(inBitMap, 55);
			//vdCTOSS_SetBitMapOff(inBitMap, 23); 
		}
		/* BDOCLG-00116: remove DE55 from void transaction - end -- jzg */

		/* BDOCLG-00566:  Offline cup de 55 is available on cup chip, should be remove - start -- jzg */
		if(srTransPara->byTransType == SALE_OFFLINE)
		{
			vdCTOSS_SetBitMapOff(inBitMap, 55);
			vdCTOSS_SetBitMapOff(inBitMap, 23); 
		}
		/* BDOCLG-00566:  Offline cup de 55 is available on cup chip, should be remove - end -- jzg */
	}
}
                             ...
                             unsigned char* uszReceData)
{
	...
	vdDebug_LogPrintf("*** HOST REQUEST ***");
    ...
    {		
		...
		vdDebug_LogPrintf("srCommFuncPoint.inSendData - inResult[%d]", inResult);
...
//amdroid-temp removed
	...
	else
    {
        ...
        else
        {
			...
			vdDebug_LogPrintf("srCommFuncPoint.inRecData - inResult[%d]", inResult);
			...
			if(strCPT.inCommunicationMode == ETHERNET_MODE)  
			{
				...
				if ( CTOS_CradleAttached() == d_NO){
                    ...
                    if(inResult <= 0)
                    {
                        if(inCheckEthernetSocketConnected() == BROKEN_PIPE)// || inCheckEthernetConnected() == ST_COMMS_DISCONNECT)  //Tine:  04JUL2019
                        {
			...
			else if(strCPT.inCommunicationMode == GPRS_MODE || strCPT.inCommunicationMode == WIFI_MODE)
			{
                if(inResult <= 0)
                {
                    if(CTOS_TimeOutCheck(TIMER_ID_3) == d_YES) /*encounter timeout*/
			...
			vdDebug_LogPrintf("Final result on receiving = [%d]", inResult);
...
#if 0
     //if(strCPT.inCommunicationMode == WIFI_MODE || strCPT.inCommunicationMode == ETHERNET_MODE)
    {
		 	   	//vdCTOS_DispStatusMessage("RECEIVING...");
		#if 0
		if(srTransRec.byTransType == SALE)
			vdDispTransTitleCardTypeandTotal(srTransRec.byTransType, "RECEIVING...");
		...
		#else
		{
			...
			vdDebug_LogPrintf("*** HOST RESPONSE ***");
					...
					else if (inPOSTErrorMessage() == TRUE && srTransRec.byTransType != PRE_AUTH && srTransRec.byTransType != SALE_OFFLINE)
                    {
                        ...
                        else if(srTransPara->byPackType != SEND_ADVICE)
                        {
							if(srTransPara->byPackType == REVERSAL && (strHDT.inFailedREV+1 >= inDelRevVal))
							{
								...
								vdDebug_LogPrintf("TINE:  test code for No Response from Bank on reversal");
							}
							else
							{
                            	vdDisplayErrorMsgResp2("No Response","from Bank","Please Try Again");
    ...
    else
    {
			...
			|| ((srTransPara->szRespCode[0] >= '0' && srTransPara->szRespCode[0] <= '9') && srTransPara->szRespCode[1] == ' ') )
        {
         ...
         else
         {
            ...
            {
            	vdDisplayMessageBox(1, 8, "Batch transfer", "", MSG_PLS_WAIT, MSG_TYPE_INFO);
				CTOS_Delay(1000);
				...
				//strcpy(szDisMsg, szTitle);
				//strcat(szDisMsg, "|");
				//strcat(szDisMsg, "BATCH TRANSFER.\nPLEASE WAIT...");
				//usCTOSS_LCDDisplay(szDisMsg);
				//CTOS_Delay(3000);
            }
            else
...
//    vduiClearBelow(8); // patrick fix code 20140421
	...
	vdDebug_LogPrintf("shCTOS_EMVSecondGenAC, inResult=[%d], srTransPara->shTransResult=[%d]", inResult, srTransPara->shTransResult);
...
//OFFline void advice
	...
	vdDebug_LogPrintf("DE03 %02x%02x%02x",uszSendData[0],uszSendData[1],uszSendData[2]);
...
int inPackIsoFunc11(TRANS_DATA_TABLE *srTransPara, unsigned char* uszSendData)
{
	...
	vdDebug_LogPrintf("--inPackIsoFunc11--");
	vdDebug_LogPrintf("srTransPara->byTransType=[%d]", srTransPara->byTransType);
     ...
     else                  
    {
        ...
        {
        	/*for UPI only*/
			if ((srTransPara->byPackType == REVERSAL || srTransPara->byPackType == VOID_REVERSAL) && (srTransPara->HDTid == 36 || srTransPara->HDTid == 37))
				sprintf(szSTAN, "%06ld", srTransPara->ulTraceNum+1);
			else
        		sprintf(szSTAN, "%06ld", srTransPara->ulTraceNum);
        }
...
int inPackIsoFunc22(TRANS_DATA_TABLE *srTransPara, unsigned char* uszSendData)
{
	...
	vdDebug_LogPrintf("saturn inPackIsoFunc22 %d- %d", srTransPara->fCUPPINEntry == TRUE, srTransRec.fEMVPIN);

	// For CUP -- sidumili
	if (srTransPara->fCUPPINEntry == TRUE || srTransRec.fEMVPIN == TRUE)
		inPINCapability = 1;
	else
		inPINCapability = 2;
    ...
    if (srTransPara->byEntryMode == CARD_ENTRY_ICC)
    {
		...
		// For CUP -- sidumili
		if (srTransPara->HDTid == 36 || srTransPara->HDTid == 37)
			sprintf(szTempEnterMode,"005%d",inPINCapability);
    ...
    else if (srTransPara->byEntryMode == CARD_ENTRY_FALLBACK)
    {
		...
		{
			if((srTransPara->szServiceCode[0] == '2') || (srTransPara->szServiceCode[0] == '6'))
				sprintf(szTempEnterMode, "080%d", inPINCapability);
			else
			{
				sprintf(szTempEnterMode, "002%d", inPINCapability);
				vdCTOS_SetTransEntryMode(CARD_ENTRY_MSR);
			}
		}
    }
    ...
    {        
		 ...
		 // For CUP -- sidumili	
		 if (srTransPara->HDTid == 36 || srTransPara->HDTid == 37)		
			sprintf(szTempEnterMode, "001%d", inPINCapability);
    }
	...
	{		
		...
		// For CUP -- sidumili	
		 if (srTransPara->HDTid == 36 || srTransPara->HDTid == 37)		
		 {
		 	sprintf(szTempEnterMode, "007%d", inPINCapability);
			if (srTransPara->bWaveSID == d_VW_SID_PAYPASS_MAG_STRIPE)
				sprintf(szTempEnterMode, "091%d", inPINCapability);
		 }
...
int inPackIsoFunc24(TRANS_DATA_TABLE *srTransPara, unsigned char* uszSendData)
{
	...
	vdDebug_LogPrintf("byTransType[%d] :: byPackType[%d] :: strTCT.fDCC[%d] :: strCDT.fDCCEnable[%d] :: inGetATPBinRouteFlag[%d] :: fBINRouteApproved[%d]",srTransPara->byTransType,srTransPara->byPackType, strTCT.fDCC,strCDT.fDCCEnable,inGetATPBinRouteFlag(),srTransPara->fBINRouteApproved);
...
//0826 - for amex host
    if((srTransRec.byTransType == PRE_AUTH) && (srTransRec.HDTid == 2 || srTransRec.HDTid == 36 || srTransRec.HDTid == 37))
...
int inPackIsoFunc53(TRANS_DATA_TABLE *srTransPara, unsigned char* uszSendData)
{
	char szTempBuf[17];
	vdMyEZLib_LogPrintf("**inPackIsoFunc53 START**");
	inDataCnt = 0;

	memset(szTempBuf,0,sizeof(szTempBuf));
	
	//if( pos_cfg.tdes )
	//if (strDCT.inPIN3des == 1)//for des pin
		wub_str_2_hex("2600000000000000", (char *)szTempBuf, 16);
	//else
	   // wub_str_2_hex("2000000000000000", (char *)szTempBuf, 16);

	DebugAddHEX("szTempBuf", szTempBuf, 16);
	
	//if( pos_cfg.tdes )
	//if (strDCT.inPIN3des == 1)//for des pin
		memcpy((char *)(BYTE *)&uszSendData[inDataCnt], szTempBuf,8);
	//else
	  //  strcpy((char *)(BYTE *)&uszSendData[inDataCnt], "2000000000000000");

	inDataCnt += 8;
	vdMyEZLib_LogPrintf("  PACK_LEN%d",inDataCnt);
	vdMyEZLib_LogPrintf("**inPackIsoFunc53 END**");
	return inDataCnt;
}
...
int inPackIsoFunc61(TRANS_DATA_TABLE *srTransPara, unsigned char* uszSendData)
{
	...
	vdDebug_LogPrintf("saturn ***inPackIsoFunc61 START***");
  ...
  if (srTransPara->byPackType == DCC_RATEREQUEST || srTransPara->byPackType == DCC_RATEREQUEST_RETRY)
  {
	 ...
	 memcpy(szTempPAN,srTransRec.szPAN,10);
...
#endif	
	vdDebug_LogPrintf("inUnPackIsoFunc37 srTransPara->byPackType: %d, srTransPara->byTransType: %d", srTransPara->byPackType, srTransPara->byTransType);
	if (srTransPara->byPackType != REVERSAL && srTransPara->byPackType != SEND_ADVICE)
    	memcpy(srTransPara->szRRN, (char *)uszUnPackBuf, 12);

	vdDebug_LogPrintf("srTransPara->szRRN=[%s]", srTransPara->szRRN);
...
int inUnPackIsoFunc38(TRANS_DATA_TABLE *srTransPara, unsigned char *uszUnPackBuf)
{
    ...
    vdDebug_LogPrintf("inUnPackIsoFunc38 srTransPara->byPackType: %d, srTransPara->byTransType: %d", srTransPara->byPackType, srTransPara->byTransType);
	...
	vdDebug_LogPrintf("srTransPara->szAuthCode=[%s]", srTransPara->szAuthCode);
...
int inPackISOEMVData(TRANS_DATA_TABLE *srTransPara, unsigned char *uszUnPackBuf)
{
    ...
    vdDebug_LogPrintf("saturn **inPackISOEMVData START**  %d", srTransPara->stEMVinfo.T9C);
...
int inPOSTErrorMessage(void)
{
	...
	DebugAddSTR("inPOSTErrorMessage",szFileName,12);  
    
    vdDebug_LogPrintf("Rever Name %s",szFileName);

	vdDebug_LogPrintf("inReversalType=[%d]", inReversalType);
...
int inSMACUnPackIsoFunc60(TRANS_DATA_TABLE *srTransPara, unsigned char *uszUnPackBuf)
{
	...
	//}
	vdDebug_LogPrintf("fSMPrintFooterMsg[%d]",strIIT.fSMPrintFooterMsg);
...
int inCUPPackIsoFunc61(TRANS_DATA_TABLE *srTransPara, unsigned char* uszSendData)
{
    vdDebug_LogPrintf("saturn **inCUPPackIsoFunc61 START**");
    inDataCnt = 0;
    uszSendData[inDataCnt++] = 0x00;
    uszSendData[inDataCnt++] = 0x02;
    memcpy((char *)&uszSendData[inDataCnt], "CC", 2);
    inDataCnt += 2;
