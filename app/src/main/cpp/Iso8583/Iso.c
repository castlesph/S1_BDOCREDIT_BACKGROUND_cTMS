...
struct BITD bittabDCC[] = {
    { 1,     0,	   ALPHANUMS,    4},
    { 2,     BYTE_2_LEN,    ALPHANUMS,    0},
    { 3,     0,	   ALPHANUMS,    16},
    { 4,     BYTE_2_LEN,    NUMERIC_,    0},
    { 6,     0,    NUMERIC_,    16},
    { 9,     0,    NUMERIC_,    10},
    { 10,    BYTE_2_LEN,	   ALPHANUMS,    0},
    { 15,    0,	   ALPHANUMS,    3},
    { 16,    0,	   ALPHANUMS,    14},
    { 17,    0,	   ALPHANUMS,    2},
    { 18,    0,	   ALPHANUMS,    11},
    { 20,    BYTE_2_LEN,	   ALPHANUMS,    0},
    { 21,    0,	   ALPHANUMS,    16},	
    { 22,    0,	   ALPHANUMS,    6},
    { 23,    0,	   ALPHANUMS,    10},
    { 24,    0,    ALPHANUMS,    3},
...
char *UnpackDCC(char *ptrBitmap, char *ptrBuf)
{
    ...
    for (i=1; i<32; i++)
    {
        if (bittest(bitmap, i))
        {
            ...
            //printf("unpack field %d\n",i);
            for (j=0; bittabDCC[j].bitn != 255; j++)
                if (bittabDCC[j].bitn == i) 
                    break;
            
            if (bittabDCC[j].bitn == 255)
                 return(NULL);

            bitp[i] = bptr;
            switch(bittabDCC[j].attrib)
            {
                case ALPHANUMS :
                    if(bittabDCC[j].format == BYTE_2_LEN)
                    {
                        //print_hex("len value",bptr,2);
						memcpy(szLen, bptr, 2); 
						len=atoi(szLen);
                    ...
                    //printf("ALPHANUM_ len=%d\n",len);
                    //vdDebug_LogPrintf("ALPHANUMS j:%d", j);
					//vdDebug_LogPrintf("bptr:%s", bptr);
					memcpy(bitDCC[i], bptr, len);
                    bptr += len;
					inDCCLen[i]=len;
					//bitp[i][len]=0x00;
                break;
				...
				case NUMERIC_:
                    ...
                    //printf("ALPHANUM_ len=%d\n",len);
                    //vdDebug_LogPrintf("NUMERIC_ j:%d", j);
					//vdDebug_LogPrintf("bptr:%s", bptr);
                    memcpy(bitDCC[i], bptr, len);
                    bptr += len;
					inDCCLen[i]=len;
...
void vdPrintParseISO(BYTE byTransType, unsigned char *pucTitle,unsigned char *buf)
{
    ...
    inPrint(szStr);
    vdMyEZLib_HexDump("TPDU:", buf, 5);
	
    vdMyEZLib_HexDump("MSG ID:", buf+5, 2);
    //inPrint("MSG ID:");
    vdMyEZLib_HexDump("BITMAP:", bitmap, 8);
    //inPrint("BITMAP:");
    //CTOS_PrinterFline(d_LINE_DOT);
    vdDebug_LogPrintf("inCCTOS_PrinterBufferOutput -- START");
    inCCTOS_PrinterBufferOutput(ONE_LINE_DOT,&stgFONT_ATTRIB,fERMTransaction);
	vdDebug_LogPrintf("inCCTOS_PrinterBufferOutput -- END");
