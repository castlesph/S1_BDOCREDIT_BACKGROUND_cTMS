...
VS_BOOL fAmountEntered = VS_FALSE;
...
{    
	...
	vdDebug_LogPrintf("--inDoAutosettlement--");
	vdDebug_LogPrintf("strTCT.fNextDayMustSettle=[%d]", strTCT.fNextDayMustSettle);
...
{
	vdDebug_LogPrintf("SATURN ENTER MAIN");
	...
	inTCTRead(1);
	vdDebug_LogPrintf("Main->strTCT.szSuperPW=[%s], strTCT.szSystemPW=[%s], strTCT.szEngineerPW=[%s], strTCT.szFunKeyPW=[%s], strTCT.szPMpassword=[%s], strTCT.szBInRoutePW=[%s]", strTCT.szSuperPW, strTCT.szSystemPW, strTCT.szEngineerPW, strTCT.szFunKeyPW, strTCT.szPMpassword, strTCT.szBInRoutePW);
	...
	if(d_OK == inMultiAP_CheckSubAPStatus())
	{
		...
		while (1)
		{
			vdDebug_LogPrintf("saturn inMultiAP_GetMainroutine");
			inMultiAP_GetMainroutine();
			//TINE -- trial for subap fix, as described in "Way to Migrate..." document item 20.
			//CTOS_Delay(strTCT.inWaitTime);
			//patrick purpose adds this code to return
			msgEnd();
			vdDebug_LogPrintf("saturn msgEnd");
			return 0;
			//TINE -- end of modified code
		}
	}
	else
	{
...
int inCTOS_IdleAmountEntry(void)
{
	int inRet=d_NO;
...
int inStartTrans(void){
	...
	inRetVal = inCTOS_GetTxnBaseAmountWithMenu();
	vdDebug_LogPrintf("saturn inStartTrans %d", inRetVal);

	

	if (inRetVal == d_OK){

		fAmountEntered = VS_TRUE;

        vdSetFirstIdleKey(srTransRec.szBaseAmount[0]); 
		if (strTCT.fDefaultCashAdvance == 1)
			inRetVal = inCTOS_CASH_ADVANCE();
		else
            inRetVal = inCTOS_WAVE_SALE();

	}

	if (d_MENU_SELECTION == inRetVal)
       inRetVal = GetMenuTransTypes();
