...
ULONG inCTOSS_CLMInitTransaction(BYTE *szAmount,BYTE *szOtherAmt,BYTE *szTransType,BYTE *szCatgCode,BYTE *szCurrCode)
{
	...
	if (CTLS_INTERNAL == inCTOSS_GetCtlsMode())
	{
		...
		vdDebug_LogPrintf("EMVCL_InitTransactionEx = %d", ulAPRtn);
	}
...
ULONG inCTOSS_CLMPollTransaction(EMVCL_RC_DATA_EX *stRCDataEx,ULONG ulMS)
{
	...
	if (CTLS_INTERNAL == inCTOSS_GetCtlsMode()) 
	{
		...
		vdDebug_LogPrintf("EMVCL_PollTransactionEx = %d", ulAPRtn);
	}
	else if (CTLS_EXTERNAL == inCTOSS_GetCtlsMode())
...
void vdCTOSS_WaveGetEMVData(BYTE *baChipData, USHORT usChipDataLen)
{
	...
	if((strIIT.fQuasiCash)  && ((memcmp(srTransRec.stEMVinfo.T84,"\xA0\x00\x00\x00\x03",5) == 0) || (memcmp(srTransRec.stEMVinfo.T84,"\xa0\x00\x00\x00\x03",5) == 0)) && ((srTransRec.byTransType == SALE) || (srTransRec.byTransType == SALE_OFFLINE)))
	...
	vdDebug_LogPrintf("SATURN 9F33: %x %x %x", srTransRec.stEMVinfo.T9F33[0],srTransRec.stEMVinfo.T9F33[1], srTransRec.stEMVinfo.T9F33[2]);
    ...
    usCTOSS_FindTagFromDataPackage(TAG_9F34_CVM, srTransRec.stEMVinfo.T9F34, &usLen, baChipData, usChipDataLen);
    //usCTOSS_FindTagFromDataPackage(TAG_5F34_PAN_IDENTFY_NO, (BYTE *)&(srTransRec.stEMVinfo.T5F34), &usLen, baChipData, usChipDataLen);
	...
	vdDebug_LogPrintf("5F34: %02x %d", byVal[0], usLen);
...
// patrick fix print application label
		...
		(!((srTransRec.stEMVinfo.szChipLabel[0] >= 'A') && (srTransRec.stEMVinfo.szChipLabel[0] <= 'Z'))))
	{
		...
		usCTOSS_FindTagFromDataPackage(TAG_50, bAppLabel, &usLen, baChipData, usChipDataLen);
...
//gcitra-get card holder number
	usCTOSS_FindTagFromDataPackage(TAG_5F20, srTransRec.szCardholderName, &usLen, baChipData, usChipDataLen);
	vdDebug_LogPrintf("5F20,szCardholderName: %s", srTransRec.szCardholderName);	
//gcitra-get card holder number
    DebugAddHEX("baChipData", baChipData, usChipDataLen);
    ...
    DebugAddHEX("T9F6E", srTransRec.stEMVinfo.T9F6E, srTransRec.stEMVinfo.T9F6E_len);
	vdDebug_LogPrintf("srTransRec.stEMVinfo.T9F6E_len[%d]",srTransRec.stEMVinfo.T9F6E_len);
}
...
//	TLVDataParse(stRCDataEx->baAdditionalData, stRCDataEx->usAdditionalDataLen);
	...
	//CTLS: set transaction type to offline sale and auth code to "Y1" - start -- jzg
	vdCTOS_SetTransType(SALE_OFFLINE);		
	strcpy(srTransRec.szAuthCode, "Y1");
	//CTLS: set transaction type to offline sale and auth code to "Y1" - end -- jzg

	//set as Online - flag to be used during advice and void - to determine that the original transaction was sale Online, but was approved offline 
	...
	else if(stRCDataAnalyze.usTransResult == d_EMV_CHIP_OFF_APPROVAL) 
	{
		...
		//CTLS: set transaction type to offline sale and auth code to "Y1" - start -- jzg
		vdCTOS_SetTransType(SALE_OFFLINE);	
		srTransRec.fCompletion = CN_FALSE; //fCompletion to false to indicate that transaction is not CHECKOUT/COMPLETION
		strcpy(srTransRec.szAuthCode, "Y1");
		//CTLS: set transaction type to offline sale and auth code to "Y1" - end -- jzg
...
//		inRet = d_NO;
	...
	if (inRet == d_OK)
	{
		...
		if (stRCDataEx->baTrack2Data[0] != ';')
		{
			...
			srTransRec.usTrack2Len = (stRCDataEx->bTrack2Len * 2) - 1;
			wub_hex_2_str(stRCDataEx->baTrack2Data,srTransRec.szTrack2Data,stRCDataEx->bTrack2Len);

			//BDO: Padding should be '0' instead of 'F' and length shouldn't include padding - start -- jzg
			vdDebug_LogPrintf("JEFF::szTrack2=[%s][%ld]", srTransRec.szTrack2Data, srTransRec.usTrack2Len);
			vdDebug_LogPrintf("JEFF::szTrack2[%d]=[0x%02X]", srTransRec.usTrack2Len, srTransRec.szTrack2Data[srTransRec.usTrack2Len]);
			if (srTransRec.szTrack2Data[srTransRec.usTrack2Len] == 0x46)
			{
				srTransRec.szTrack2Data[srTransRec.usTrack2Len] = 0x30;
				srTransRec.usTrack2Len = srTransRec.usTrack2Len - 2;
			}
			//BDO: Padding should be '0' instead of 'F' and length shouldn't include padding - end -- jzg
		}
		else
		{
		...
		if (srTransRec.stEMVinfo.T9F34[0] == 0x3F)
			srTransRec.bWaveSCVMAnalysis = d_CVM_REQUIRED_NOCVM;
		...
		usCTOSS_FindTagFromDataPackage(TAG_5F34_PAN_IDENTFY_NO, (BYTE *)&(srTransRec.stEMVinfo.T5F34), &usLen,	stRCDataEx->baChipData, stRCDataEx->usChipDataLen);

		/* CTLS: if AMEX ExpressPay 3.0 accept 0 length and 00 value for tag 5F34 - start -- jzg */
		if((srTransRec.bWaveSID == d_VW_SID_AE_EMV) && (usLen <= 0))
			srTransRec.stEMVinfo.T5F34_len = 1;
		else
		/* CTLS: if AMEX ExpressPay 3.0 accept 0 length and 00 value for tag 5F34 - end -- jzg */
			srTransRec.stEMVinfo.T5F34_len = usLen;
...
void vdCTOSS_WaveCheckRtCode(ULONG ulAPRtn)
{
	...
	{
//		vdDisplayErrorMsg(1, 8, "Card not Supported"); //BDO UAT 0001: API is giving out d_EMVCL_RSP_ID_ERROR instead of d_EMVCL_RX_TIMEOUT during time out -- jzg
	}
	/* Issue# 000203 - end -- jzg */
	else
	{
		vdDisplayErrorMsg(1, 8, "Please Try Again");
		//vdDisplayErrorMsg(1, 8, "Card not Supported"); //sidumili: Fix on Issue#: 000223
	}
}
...
//return of if ctls is turned off
	if (NO_CTLS == inCTOSS_GetCtlsMode())
		return d_OK;
...
USHORT usCTOSS_CtlsV3Trans(BYTE *szAmount,BYTE *szOtherAmt,BYTE *szTransType,BYTE *szCatgCode,BYTE *szCurrCode,EMVCL_RC_DATA_EX *stRCDataEx)
{
	...
	vdDebug_LogPrintf("saturn usCTOSS_CtlsV3Trans");
...
//	vdDebug_LogPrintf("-----szCurrCode=[%s]-----",strCTLSTransData.szCurrCode);
	...
	vdDebug_LogPrintf("saturn **usCTOSS_CtlsV3Trans**byPinPadMode=[%d],byPinPadType=[%d],byPinPadPort=[%d],GetCtlsMode=[%d]",strTCT.byPinPadMode,strTCT.byPinPadType,strTCT.byPinPadPort,inCTOSS_GetCtlsMode());
...
/*
		...
		{
			usResult = inMultiAP_RunIPCCmdTypes("com.Source.SHARLS_CTLS.SHARLS_CTLS", d_IPC_CMD_CTLS_TRANS, bInBuf, usInLen, bOutBuf, &usOutLen);
		}
*/
		{
			EntryOfStartTrans_1:
			usResult = inMultiAP_RunIPCCmdTypes("com.Source.SHARLS_CTLS.SHARLS_CTLS", d_IPC_CMD_CTLS_TRANS, bInBuf, usInLen, bOutBuf, &usOutLen);
			DebugAddHEX("ulAPRtn", bOutBuf, 10);

			vdDebug_LogPrintf("saturn usResult=[%d],usResult=[%x] bOutBuf[4] [%02X][%ld]",usResult, usResult, bOutBuf[4], bOutBuf[4]);
			if (d_OK == usResult)
			{
				unsigned long ulAPRtn;

				ulAPRtn = ((bOutBuf[1] ) | (bOutBuf[2] << 8) | (bOutBuf[3] << 16) | (bOutBuf[4] << 24));
				vdDebug_LogPrintf("saturn ulAPRtn1=[%08X]\n", ulAPRtn);

				/*handle second tap case*/
				if ((bOutBuf[4]) == 0xA0)
				{
					if ((ulAPRtn == d_EMVCL_RC_SEE_PHONE) || (ulAPRtn ==d_EMVCL_RC_SEE_PHONE2))
					{
						...
						CTOS_Delay(3000);
						InsertCardUI();
						goto EntryOfStartTrans_1;
					}
					else if (ulAPRtn == d_EMVCL_RC_DEK_SIGNAL)
					{
						usCTOSS_BackToProgress(" ");
						vdDisplayErrorMsg(1, 8, "  !SEE PHONE!      ");
						CTOS_Delay(3000);
					}
					else if (ulAPRtn == d_EMVCL_RC_FALLBACK)
					{
						usCTOSS_BackToProgress(" ");
						vdDisplayErrorMsg(1, 8, "INSERT or SWIPE CARD");
						CTOS_Delay(3000);
					}
					else if (ulAPRtn == d_EMVCL_RC_NO_CARD)
					{
						usCTOSS_BackToProgress(" ");
						vdDisplayErrorMsg(1, 8, "Please Present Card");
						CTOS_Delay(3000);
					}
					else if (ulAPRtn == d_EMVCL_RC_MORE_CARDS)
					{
						usCTOSS_BackToProgress(" ");
						vdDisplayErrorMsg(1, 8, "Present one card only");
						CTOS_Delay(3000);
					}
					else if (ulAPRtn == d_EMVCL_TRY_AGAIN)
					{
						usCTOSS_BackToProgress(" ");
						vdDisplayErrorMsg(1, 8, "TRY AGAIN         ");
						CTOS_Delay(3000);
					}
					else if (ulAPRtn != d_EMVCL_RC_FAILURE)
					{
						usCTOSS_BackToProgress(" ");
						CTOS_LCDTPrintXY(1, 4, "!UN-SUPPORT CARD! ");
						CTOS_LCDTPrintXY(1, 5, "INSERT or SWIPE CARD");
						CTOS_LCDTPrintXY(1, 6, "USE OTHER CARD    ");
					}
	...
	else
	{
		...
		usResult = inMultiAP_RunIPCCmdTypes("com.Source.SHARLS_CTLS.SHARLS_CTLS", d_IPC_CMD_CTLS_TRANS, bInBuf, usInLen, bOutBuf, &usOutLen);
		vdDebug_LogPrintf("saturn usResult=[%d],usResult=[%x] bOutBuf[0] [%02X]",usResult, usResult, (bOutBuf[4]));
		...
		if (d_OK == usResult)
		{
			...
			ulAPRtn = ((bOutBuf[1] ) | (bOutBuf[2] << 8) | (bOutBuf[3] << 16) | (bOutBuf[4] << 24));
			vdDebug_LogPrintf("saturn ulAPRtn2=[%08X]\n", ulAPRtn);
    ...
    vdDebug_LogPrintf("saturn **usCTOSS_CtlsV3Trans [%d]**", usResult);
    if(d_OK == usResult)
    {
    	...
    	vdDebug_LogPrintf("saturn usCTOSS_CtlsV3Trans[%x][%c][%d] ",usStatus,usStatus,usStatus);
		if (usStatus == d_SUCCESS)
		{
			usResult = d_OK;
			if ((strTCT.byPinPadMode == 0) && (strTCT.byPinPadType == 3) && CTLS_V3_INT_SHARECTLS != inCTOSS_GetCtlsMode())
			{
				vdDebug_LogPrintf("saturn status [%d]**", strCTLSTransData.status);
			}
			else
			{
...
*/
		{
			...
			vdDebug_LogPrintf("usResult=[%d],usResult=[%x] bOutBuf[4] [%02X][%ld]",usResult, usResult, bOutBuf[4], bOutBuf[4]);
			if (d_OK == usResult)
			{
				unsigned long ulAPRtn;

				ulAPRtn = ((bOutBuf[1] ) | (bOutBuf[2] << 8) | (bOutBuf[3] << 16) | (bOutBuf[4] << 24));
				vdDebug_LogPrintf("ulAPRtn=[%08X]\n", ulAPRtn);
	...
	else
	{
		...
		usResult = inMultiAP_RunIPCCmdTypes("com.Source.SHARLS_CTLS.SHARLS_CTLS", d_IPC_CMD_CTLS_TRANS, bInBuf, usInLen, bOutBuf, &usOutLen);
		vdDebug_LogPrintf("usResult=[%d],usResult=[%x] bOutBuf[0] [%02X]",usResult, usResult, (bOutBuf[4]));
...
//	TLVDataParse(stRCDataEx->baAdditionalData, stRCDataEx->usAdditionalDataLen);
	...
	*/
	...
	vdDebug_LogPrintf("test wave analyze %d", stRCDataAnalyze.usTransResult);
	...
	else if(stRCDataAnalyze.usTransResult == d_EMV_CHIP_OFF_DECLINED) 
	{
		...
		strcat(szDisMsg, "|");
		strcat(szDisMsg, "OFFLINE DECLINED");
		vdDisplayErrorMsg(1,1,szDisMsg);

		return CTLS_EMV_OFFLINE_DECLINE;
		//inRet = d_NO;
//		inRet = d_OK;
	}
	else 
...
//			}			
		...
		/* CTLS: if AMEX ExpressPay 3.0 accept 0 length and 00 value for tag 5F34 - start -- jzg */
		if((srTransRec.bWaveSID == d_VW_SID_AE_EMV) && (srTransRec.stEMVinfo.T5F34_len <= 0))
			srTransRec.stEMVinfo.T5F34_len = 1;
	}
...
int inCTOSS_CheckCVMOnlinepin(void)
{
	...
	vdDebug_LogPrintf("AAA - inCTOSS_CheckCVMOnlinepin START");
...
int inCTOSS_CheckCVMAmount(void)
{
	...
	return d_OK; // bypass for certification only.

#if 0
    if ((srTransRec.fDCCOptin== TRUE) || (srTransRec.fInstallment == TRUE) || (srTransRec.byEntryMode==CARD_ENTRY_MANUAL) 
    || (srTransRec.byTransType == REFUND) || (srTransRec.byTransType == SALE_OFFLINE) || (srTransRec.byTransType == PRE_AUTH)
    || (srTransRec.byTransType == PREAUTH_COMP) || (srTransRec.byTransType == PREAUTH_VOID) || (srTransRec.byTransType == PREAUTH_VER)
    || (srTransRec.byTransType == SALE_TIP)|| (srTransRec.byOrgTransType == SALE_TIP) ||(srTransRec.byOrgTransType == REFUND)||(srTransRec.byOrgTransType == SALE_OFFLINE)|| ((srTransRec.byEntryMode==CARD_ENTRY_FALLBACK) && srTransRec.IITid!=AMEX_ISSUER))
         return d_NO;
#endif
	memset(szTemp,0x00,sizeof(szTemp));
	wub_hex_2_str(srTransRec.szTotalAmount, szTemp, AMT_BCD_SIZE);
	vdDebug_LogPrintf("saturn  inCTOSS_CheckCVMAmountszTotalAmount =[%s]-----",szTemp);
	
	if (strlen(strIIT.szCVMLimit)> 0)
	{
		vdDebug_LogPrintf("saturn inCTOSS_CheckCVMAmount - szCVMLimit =[%s]",strIIT.szCVMLimit);
	}

		if (atol(szTemp) <= atol(strIIT.szCVMLimit))
		{
...
int inCTOSS_CheckCVMAmount(void)
{
	...
	vdDebug_LogPrintf("SATURN - inCTOSS_CheckCVMAmount START");
	...
	vdDebug_LogPrintf("saturn AIDlen =[%d]-----",AIDlen);
   vdPCIDebug_HexPrintf("saturn AID",AID,AIDlen);
   ...
   // if ((srTransRec.fDCCOptin== TRUE) || (srTransRec.fInstallment == TRUE) || (srTransRec.byEntryMode==CARD_ENTRY_MANUAL) 
	...
	memset(szTemp,0x00,sizeof(szTemp));
	wub_hex_2_str(srTransRec.szTotalAmount, szTemp, AMT_BCD_SIZE);
	vdDebug_LogPrintf("saturn szTotalAmount =[%s] %ld-----",szTemp, atol(szTemp));
		vdDebug_LogPrintf("saturn szCVMLimit =[%s]-----%ld",strIIT.szCVMLimit, atol(strIIT.szCVMLimit));

	
	if (strlen(strIIT.szCVMLimit)> 0)
	//if (strlen("200000")> 0)
	{
		//vdDebug_LogPrintf("AAA - szCVMLimit =[%s]",strIIT.szCVMLimit);
	}

	if ((memcmp(AID,"\xA0\x00\x00\x03\x33\x01\x01\x02",AIDlen) == 0) || (memcmp(AID,"\xA0\x00\x00\x03\x33\x01\x01\x03",AIDlen) == 0))
    {
		if (atol(szTemp) <= atol(strIIT.szCVMLimit))
		{
			vdDebug_LogPrintf("SATURN - inCTOSS_CheckCVMAmount ret d_OK");
		     return d_OK;
		}
	}
    vdDebug_LogPrintf("SATURN - inCTOSS_CheckCVMAmount ret d_NO");
	return d_NO;
