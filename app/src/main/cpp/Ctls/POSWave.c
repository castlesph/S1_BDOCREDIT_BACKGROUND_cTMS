...
*/
   ...
   if(srTransRec.usTerminalCommunicationMode == DIAL_UP_MODE && srTransRec.fBINVer == TRUE)
   {
    ...
    if(fEntryCardfromIDLE == TRUE && (srTransRec.byEntryMode == CARD_ENTRY_ICC || srTransRec.byEntryMode == CARD_ENTRY_MSR)){		
...
#if 0
		//tine:  17DEC2019  temporarily remove, not yet fixed for android terminal
		...
		{
			if((strcmp(strHDT.szAPName,V5S_BDODEBIT) == 0) && strCST.inCurrencyIndex != CURRENCY_PHP)
			{
...
//for enhance the treansaction speed
	...
	//tine: android - fix display overlap with idle screen
    ...
    if(d_OK != inRet)
        return inRet;
	else if((srTransRec.usTerminalCommunicationMode == GPRS_MODE || srTransRec.usTerminalCommunicationMode == WIFI_MODE || srTransRec.usTerminalCommunicationMode == ETHERNET_MODE)
...
SWIPE_AGAIN:
		...
		// uncomment to prompt -- sidumii
		//vdDisplayMessageBox(1, 8, "Card reading", "complete", "", MSG_TYPE_SUCCESS);
		//CTOS_Delay(10000);
		
		//add emv offline decline - for CTLS certification
		if(inRet == d_USER_CANCEL || inRet == d_NO || inRet == CTLS_EMV_OFFLINE_DECLINE)
		{
			return d_NO;
		}

	// Card reading complete...
	if(get_env_int("CARDCHECK"))
		inDisplayMessageBoxWithButtonConfirmation(1,8,"Card reading complete.","Card number",srTransRec.szPAN,"",MSG_TYPE_INFO, BUTTON_TYPE_NONE_OK);

	vdDebug_LogPrintf("saturn return %d", inRet);

	//for emv certification only
	if (inRet == CTLS_EXPIRED_CARD)
		 inRet = d_OK;
   ...
   // else if(inRet == CTLS_ANALYZE_ERROR || inRet == CTLS_EXPIRED_CARD)//Prompt user to Swipe/Insert card on Analyze error or Expired card on CTLS.
    else if(inRet == CTLS_ANALYZE_ERROR)//Prompt user to Swipe/Insert card on Analyze error or Expired card on CTLS.

	{
		inGblCtlsErr=2;
		vdResetAllCardData();
		vdDebug_LogPrintf("saturn fallback call 1: Cardentry faild ctls fallback");
		inRet = inCTOS_GetCardFieldsFailedCtlsFallback();
		if(d_OK != inRet)
        	return inRet;
	}		
	else if (d_OK != inRet)
...
//inInitializePinPad();
	...
	//vdDebug_LogPrintf("inCTOS_WaveFlowProcess:	inGetIPPPinEx -- START");
	//inRet = inGetIPPPinEx();
	//if(d_OK != inRet)
	//	return inRet;
//test-remove
	...
	else
	{
		...
		// commented: error message already display on inCTOS_DisplayCardTitle function -- sidumili
		#if 0
		//Tine:  added code to handle user cancel/timeout from android terminal
		if(0xFF == inRet){
			...
			CTOS_Beep();
	        return inRet;
    	}else if(d_USER_CANCEL == inRet){
			//Tine: 08MAY2019
	        //memset(szTitle, 0x00, sizeof(szTitle));
	        //szGetTransTitle(srTransRec.byTransType, szTitle);
	        //strcpy(szDisMsg, szTitle);
	        //strcat(szDisMsg, "|");
	        //strcat(szDisMsg, "TRANSACTION CANCELLED");
	        //usCTOSS_LCDDisplay(szDisMsg);
	        vdDisplayMessageBox(1, 8, "", "TRANSACTION CANCELLED", "", MSG_TYPE_WARNING);
	        CTOS_Beep();
	        CTOS_Delay(1000);
	        CTOS_Beep();
			vdRemoveCard();
...
//Tine:  07May2019 -- Disable UpdateTxnTotalAmount, already performed earlier in processflow.
#if 0	
	inRet = inCTOS_UpdateTxnTotalAmount();
	if(d_OK != inRet)
			return inRet;
...
BINROUTE_AGAIN:
	...
	if(memcmp(srTransRec.stEMVinfo.T84,"\xA0\x00\x00\x06\x35",5) == 0 || memcmp(srTransRec.stEMVinfo.T84,"\xa0\x00\x00\x06\x35",5) == 0)//Bancnet AID
	{
		...
		fBinRoute=FALSE;		
		srTransRec.HDTid = strCDT.HDTid = 53;
    ...
    vdDebug_LogPrintf("strTCT.fATPBinRoute:%d, strCDT.fPANCatchAll:%d, fBinRoute:%d", strTCT.fATPBinRoute, strCDT.fPANCatchAll, fBinRoute);
...
ENTER_AGAIN:	
					...
					vdDebug_LogPrintf("saturn fallback call 4: Cardentry faild ctls fallback");

					inRetVal = inCTOS_GetCardFieldsFailedCtlsFallback();
					if(d_OK != inRetVal)
					{
...
//if (chGetInit_Connect() != 1)
{
	...
	vdDebug_LogPrintf("log1: strCDT.HDTid = [%d], srTransRec.MITid = [%d]",strCDT.HDTid,srTransRec.MITid);  //Tine:  for debug purposes only
    //if (inMMTReadRecord(strCDT.HDTid,srTransRec.MITid) != d_OK) // Also check if Merchant using host is enabled
    if (inMMTReadRecordEx(strCDT.HDTid,srTransRec.MITid) != d_OK) // Also check if Merchant using host is enabled
    {
		vdDebug_LogPrintf("log2: strCDT.HDTid = [%d], srTransRec.MITid = [%d]",strCDT.HDTid,srTransRec.MITid);  //Tine:  for debug purposes only
        vdDisplayErrorMsgResp2(" ", "TRANSACTION", "NOT ALLOWED");
		inDatabase_TerminalCloseDatabase();
        return(d_NO);
    }	
	...
	if(strTCT.fDCC	&& srTransRec.byEntryMode != CARD_ENTRY_WAVE)
    {
	...
	//for cup
#if 1
		vdDebug_LogPrintf("saturn check for CUP ");
		if (srTransRec.HDTid == 36 || srTransRec.HDTid == 37){

		vdDebug_LogPrintf("saturn bypass pin");
			
		inRet = inCTOS_ByPassPIN();
		if(d_OK != inRet)
			return inRet;


		//This will be called for SWIPE and FALLBACK
    	//if(srTransRec.fCUPPINEntry == TRUE) 
    	//testlang
    	 if ((srTransRec.fCUPPINEntry == TRUE) && (srTransRec.byEntryMode != CARD_ENTRY_ICC || srTransRec.byEntryMode != CARD_ENTRY_WAVE)) 

    	{
        	inInitializePinPad();
        
        	inRet = inGetIPPPinEx();	//android - removed
	...
	//inCTOSS_CheckMemoryStatusEx("get cvv");
		}
#endif
    ...
    if (fDualBrandedCard == FALSE && fNoEMVProcess == FALSE)
	{
     ...
     if (srTransRec.fEMVPINEntered != TRUE){
		...
		vdDebug_LogPrintf("SATURN ::inCTOS_WaveFlowProcess 9F34 = [%02X %02X %02X]", srTransRec.stEMVinfo.T9F34[0], srTransRec.stEMVinfo.T9F34[1], srTransRec.stEMVinfo.T9F34[2]);
		//if((srTransRec.stEMVinfo.T9F34[0] & 0x0F) == 0x02)
		//if(((srTransRec.stEMVinfo.T9F34[0] & 0x0F) == 0x02) && (inCTOSS_CheckCVMAmount()== d_NO))
		vdDebug_LogPrintf("SATURN :: %02X", stRCDataAnalyze.bCVMAnalysis);
		if(stRCDataAnalyze.bCVMAnalysis == d_CVM_REQUIRED_ONLPIN && inCTOSS_CheckCVMAmount()== d_NO)
		{
			//inRet = inGetIPPPin();	//android - removed
        		inRet = inGetIPPPinEx();

				vdDebug_LogPrintf("saturn inGetIPPPinEx return %d ", inRet);
				if(d_OK != inRet)
				 	return inRet;
				else{
					...
					vdDebug_LogPrintf("saturn test lang ");
				}
		}
}
#endif
	...
	vdDebug_LogPrintf("saturn before connect");
   ...
   if(srTransRec.usTerminalCommunicationMode == GPRS_MODE || (srTransRec.usTerminalCommunicationMode == WIFI_MODE && fAutoSMACLogon == FALSE))
   {
    ...
    if(d_OK != inRet)
	{
		...
		if (strlen(srTransRec.szRespCode) <= 0)
		{
			...
			vdDebug_LogPrintf("srTransRec.szECRRespCode = [%s]", srTransRec.szECRRespCode);
		}
		else
		{
	...
	vdDebug_LogPrintf("strGBLVar.fGBLvDCC=[%d]", strGBLVar.fGBLvDCC);
	if (strTCT.fDCC && strGBLVar.fGBLvDCC)
...
int inCTOS_WAVE_SALE(void)
{
	...
	//Tine:  08MAY2019 --  Include condition to handle USER CANCEL/TIMEOUT from android terminal.
	//if(inRet == d_USER_CANCEL || inRet == 0xFF)
		//return 1;
...
void vdCTOS_InitWaveData(void)
{
	...
	usCTOSS_LCDDisplay("CONTACTLESS|CONFIGURE READER...");
...
void vdCTOS_InitWaveData(void)
{
	...
	//usCTOSS_LCDDisplay("CONTACTLESS|CONFIGURE READER...");
	vdDisplayMessageBox(1, 8, "Contactless", "configure reader", MSG_PLS_WAIT, MSG_TYPE_WAIT);
