...
extern int inReadDynamicMenu(int inIndex);
...
int inCTOS_SelectHostSetting(void)
{
	...
	BYTE  x = 1;
	BYTE key;
	char szHeaderString[100] = "SELECT HOST";
	...
	{
		vdDebug_LogPrintf("szHostName[%d]=[%d]", inLoop, szHostName[inLoop]);
...
int vdCTOS_IPConfig(void)
{
	...
	else if (key > 0)
	{
		if (key == 1)	
		{
			...
			inIPHeader = HEX_EXCLUDE_LEN;
		}
		else if (key == 2)	 
		{
			inCommMode = WIFI_MODE;
			inIPHeader = BCD_EXCLUDE_LEN;
		}
		...
		// update TMSEX comm mode
		//inTMSEXRead(1);
		//memset(strTMSEX.szTMSMode, 0x00, sizeof(strTMSEX.szTMSMode));
		//sprintf(strTMSEX.szTMSMode, "%d", inCommMode);
		//inTMSEXSave(1);

		//usCTOSS_LCDDisplay(" |Configuring Comms\nPlease wait...");
		//vdDisplayMessageStatusBox(1, 8, "Configuring comms", MSG_PLS_WAIT, MSG_TYPE_PROCESS);	
		vdDisplayMessageBox(1, 8, "Configuring comms", "", MSG_PLS_WAIT, MSG_TYPE_PROCESS);
		CTOS_Delay(1000);
		
		inUpdateCommsMode(inCommMode, inIPHeader);
...
int vdCTOS_DeleteBatch(void)
{
	...
	vdCTOS_SetTransType(CLEAR_BATCH);
	...
	if(inResult == d_OK)
	{
		...
		inDisplayMessageBoxWithButtonConfirmation(1,8,"","Clear batch","Record done","",MSG_TYPE_INFO, BUTTON_TYPE_NONE_OK);
		CTOS_Delay(1000);
...
int vdCTOS_DeleteReversal(void)
{
	...
	//vduiLightOn();
	vdCTOS_SetTransType(CLEAR_REVERSAL);
	...
	shHostIndex = inCTOS_NewSelectHostSettingWithIndicator(2, "Clear Reversal", "Select Host");
	if (shHostIndex == -1)
		return d_NO;
	...
	if((inResult = inMyFile_CheckFileExist(szFileName)) < 0)
	{
		...
		inDisplayMessageBoxWithButtonConfirmation(1,8,"","No reversal exist","","",MSG_TYPE_INFO, BUTTON_TYPE_NONE_OK);
		CTOS_Beep();
		CTOS_Delay(1000);
		return d_NO;
	}
	...
	if(inResult == d_OK)
	{
		...
		inDisplayMessageBoxWithButtonConfirmation(1,8,"","Clear reversal","Record done","",MSG_TYPE_INFO, BUTTON_TYPE_NONE_OK);
		CTOS_Beep();
		CTOS_Delay(1000);
...
void vdCTOS_TipAllowd(void)
{
	...
	// for android
	vdDisplayErrorMsg(1, 8, "FUNCTION NOT ALLOWED");
	return;
...
int inCTOS_ManualSettle(void)
{
	...
	if (fAUTOManualSettle == TRUE){
		...
		shHostIndex = inCTOS_NewSelectHostSettingWithIndicator(1, "Clear Batch", "Select Host");
		if (shHostIndex == -1)
			return d_NO;
	}
	...
	PRINT_AGAIN:
	inResult=inCTOS_PrintSettleReport(TRUE);
	if(d_OK != inResult){
...
//printter check
	...
	while(1)
	{
		...
		if(inRet==d_PRINTER_PAPER_OUT)
		{
			...
			strcat(szDisMsg, "INSERT PAPER|");
			strcat(szDisMsg, "AND PRESS OK TO PRINT...");
			CTOS_Beep();
...
//printer check
	...
	if(d_OK == inResult){
		...
		//usCTOSS_LCDDisplay("CLEAR BATCH | BATCH DELETED");
		//CTOS_Beep();
		//CTOS_Delay(2000);
		vdDisplayErrorMsgResp2("", "CLEAR BATCH", "BATCH DELETED");
		...
		vdPromptManualPosting(); // prompt for manual posting -- sidumili
...
int inSelectTelcoSetting()
{
	...
	if (key > 0)
	{
		...
		vdDisplayMessageBox(1, 8, "Configuring telco", "", MSG_PLS_WAIT, MSG_TYPE_PROCESS);
		CTOS_Delay(1000);
		...
		#else
			inCPT_IPUpdate(strGPT.szGPTPriTxnHostIP, strGPT.szGPTSecTxnHostIP, strGPT.szGPTPriSettlementHostIP, strGPT.szGPTSecSettlementHostIP); // update ip only -- sidumili
			inCPT_PortUpdate(strGPT.inGPTPriTxnHostPortNum, strGPT.inGPTSecTxnHostPortNum, strGPT.inGPTPriSettlementHostPort, strGPT.inGPTSecSettlementHostPort); // update port only -- sidumili

			inPHQRIPUpdate(1, strGPT.szGPTPriTxnHostIP, strGPT.szGPTSecTxnHostIP); // Update PHQR ip only -- sidumili

			inBDOPayRead(1); // Read BDOPay -- sidumili
			vdDebug_LogPrintf("BDOPay[Before]->szProxyURL=[%s],inProxyPort=[%d]", strBDOPAY.szProxyURL, strBDOPAY.inProxyPort);
			memset(szPriTxnHostIP, 0x00, sizeof(szPriTxnHostIP));
			sprintf(szPriTxnHostIP, "%s:%d", strGPT.szGPTPriTxnHostIP, strBDOPAY.inProxyPort);
			vdDebug_LogPrintf("BDOPay[After]->szProxyURL=[%s]", szPriTxnHostIP);
			inBDOPayIPUpdate(1,szPriTxnHostIP); // Update BDOPay szProxyURL only -- sidumili

			// update TMSEX table (CTMS)
			inTMSEXRead(1);
			strcpy(strTMSEX.szTMSHostIP, strGPT.szGPTTMSHostIP);
			strcpy(strTMSEX.szTLSHostIP, strGPT.szGPTTLSHostIP);
			strTMSEX.inTMSHostPortNum = strGPT.inGPTTMSHostPortNum;
			strTMSEX.inTLSHostPortNum = strGPT.inGPTTLSHostPortNum;
			inTMSEXSave(1);
		#endif
		//-------------------------------------------------------------
		...
		if (key != 4){
			...
			if (fSelectOK){
				...
				*/
				...
				vdDisplayMessageBox(1, 8, "Configuring comms", "", "Done!", MSG_TYPE_PROCESS);
				CTOS_Beep();
			    CTOS_Delay(1000);
			}
			else{
...
int vdDisplaySetup(void)
{
	...
	vdDebug_LogPrintf("--vdDisplaySetup--");
	inMenuNumRecs = inReadDynamicMenu(2);
...
void vdCTOS_FunctionKey(void)
{
	...
	//ret = InputString(1, 8, 0x00, 0x02, strOut, &usLen, 1, d_INPUT_TIMEOUT);
	ret = InputString(1, 8, 0x00, 0x02, strOut, &usLen, 1, strTCT.inIdleTimeOut * 100);
	if(ret == d_KBD_ENTER)
	{
...
int vdCTOS_ChangeMenuID()
{
	...
	while(1)
	{
		...
		CTOS_LCDTPrintXY(1, 7, "PLEASE ENTER [1..106]");
...
int inCTOS_SelectHostSettingWithIndicator(int inIndicator)
{
	...
	BYTE szMessage[9999 + 1] = {0};
	int HDTid = 0;

	vdDebug_LogPrintf("--inCTOS_SelectHostSettingWithIndicator--");
...
/*values for inIndicator = 1=check if batch has record; 2=if there's pending reversal*/
	...
	inSetColorMenuMode();
	key = MenuDisplay(szHeaderString, strlen(szHeaderString), bHeaderAttr, iCol, x, szHostMenu, TRUE);
	vdDebug_LogPrintf("xkey[%d] xHostID[%d]", key, inCPTID[key-1]);
	#else
		for (inLoop = 0; inLoop < 150; inLoop++)
		{
			if (szHostName[inLoop][0]!= 0)
	...
	// menu title
	memset(szTitle, 0x00, sizeof(szTitle));
	sprintf(szTitle, "%s", (strlen(strGBLVar.szGBLvMenuTitle) > 0 ? strGBLVar.szGBLvMenuTitle : ""));
	vdDebug_LogPrintf("szTitle=[%s]", szTitle);
	
	memset(szMessage, 0x00, sizeof(szMessage));
	sprintf(szMessage, "%d|%d|%s|%s^", MENU_TEXT_VIEW, MENU_1_COL_COUNT, szTitle, "Select host");
	...
	key = inGetMenuKeySelected(szHostMenu, strGBLVar.szGBLvProcessDesc);
    vdDebug_LogPrintf("inCTOS_SelectHostSettingWithIndicator, inGetMenuKeySelected=[%d]", key);
	
	if(key > 0)
	{
		// get hdtid
		purge_chr(strGBLVar.szGBLvProcessDesc, '*');
		HDTid = getHostIndex(strGBLVar.szGBLvProcessDesc);
		vdDebug_LogPrintf("key[%d] HDTid[%d]", key, HDTid);
		srTransRec.HDTid = HDTid;
		strHDT.inHostIndex = HDTid;
		inHDTRead(HDTid);
		inCPTRead(HDTid);
		strcpy(srTransRec.szHostLabel, strHDT.szHostLabel);
		inCSTRead(strHDT.inCurrencyIdx); /*to load Currence and amount format*/
...
int inCTOS_SelectPreAuthHostSettingWithIndicator(int inIndicator)
{
	...
	vdDebug_LogPrintf("--inCTOS_SelectPreAuthHostSettingWithIndicator--");
/*values for inIndicator = 1=check if batch has record; 2=if there's pending reversal*/
...
int inCTOS_SelectIssuer(void)
{
	BYTE bHeaderAttr = 0x01+0x04, iCol = 1;
	BYTE  x = 1;
	BYTE key;
	char szHeaderString[100] = "SELECT ISSUER";
	char szHostMenu[HOSTNAME_BUFFER_SIZE];
	char szHostName[100][100];
	int inCPTID[100];
	int inLoop = 0;

	vdDebug_LogPrintf("--inCTOS_SelectIssuer--");
	...
	inIITReadIssuerName(szHostName, inCPTID);
