...
void vdMyEZLib_Printf(const char* fmt, ...)
{
    ...
    inCCTOS_PrinterBufferOutput(printBuf,&stgFONT_ATTRIB,fERMTransaction);
}
...
void vdRenameISOLog()
{
    ...
    if((ISOFile = fopen(ISOLOG_FILE, "r+t")) != NULL)
    {
        for (inCount = 1; inCount <= 7; ++inCount)
        {
            ...
            char szFileName[]="ISO.LOG", szFileDir[]="/data/data/pub/";
            ...
            if(access(szFileDir, 0)!=0)
            {
                ...
                inResult = rename("/data/data/pub/ISO.LOG", szFileDir);
...
void vdMyEZLib_HexDump(unsigned char *msg, unsigned char *fmt, short inlen)
{
    ...
    for ( i=0 ;i<inlen; i++)
    {
        ...
        if(index == 13)
        {
            ...
            #if 0
            memset(printBuf2, 0x00, sizeof(printBuf2));
            if(inLabelFlag == TRUE)
            {
				inLabelFlag=FALSE;
				sprintf(printBuf2, "%s %s", msg, printBuf);
            }
			else
			{
				sprintf(printBuf2, "   %s", printBuf);
			}
			inPrint(printBuf2);
			#else
			if(inLabelFlag == TRUE)
...
int GetDelimitedString(char *buf, int fldnum, unsigned char sep, char *dest)
{
	...
	for (i=0;*ptr != sep && *ptr && i<254;i++) *dest++ = *ptr++;
	*dest = '\0';
...
// ToUpper Case -- sidumili
char* SetToUpperCase(char* szString)
{
	int i = 0;
