...
typedef struct
{
	...
	BYTE szGBLvSelectedValue[6000+ 1];
	BYTE szGBLvProcessDesc[10 + 1]; 
	INT  inGBLRecordCount;
	INT fGBLOptionPrintUpload;
	BOOL fGBLContinuePrint;
	BOOL fGBLPrintReport;
	BOOL fGBLUploadReport;
	BOOL fGBLThumbnail;
	BOOL fGBLERMConnect;
	BOOL fGBLRcptPrntFlag;
	BOOL fGBLReptPrntFlag;
	BOOL fGBLAppHeaderCheckOnce;
	INT inGBLBPTBillerID;
	BYTE szGBLvAcntInfo[500+ 1];
	BYTE szGBLvWorkInfo[500+ 1];
	BYTE szGBLvImageInfo[500+ 1];
	BYTE szGBLvSignatureInfo[500+ 1];
	INT inGBLKeyType;
	BOOL fGBLTransReceipt;
	BOOL fGBLSettleAll;
	INT inGBLHDTid;
	BOOL fGBLErrorReceipt;
	BYTE szGBLvMenuTitle[40 + 1];
	BYTE szGBLvPrintTitle[40 + 1];
	BYTE szGBLvOutBuffer[512 + 1];
	BOOL fGBLvDCC;
	BYTE szGBLButtonItemLabel[110];
	BYTE szGBLButtonItemFunc[110];
	BYTE szGBLSubMenuName[110];
...
typedef struct
{
	BYTE szWriteData[9999 + 1];
	BYTE szWriteTemp[9999 + 1];
	BYTE szTemp[6000 + 1];
	int inWriteSize;
...
typedef struct
{
	BYTE	MerchantID[100+1] ;  
	BYTE	ScanQRURL[100+1] ;
	BYTE	GenerateQRURL[100+1] ;
	BYTE	GetStatusURL[100+1];
	BYTE	VoidURL[100+1];
	...
	char	szRefID[6+1];
	char	szPAN[16+1];
	char	szFailureMsg1[32 + 1];
	char	szFailureMsg2[32 + 1];	
	char	szFailureMsg3[32 + 1];
	char szProxyURL[30+1];
	BYTE szAppKey[50 + 1];
	char szAppSecret[50+1];
	char szMerchantAPIUser[30+1];
	char szMerchantAPIKey[100+1];
	char szTranStatusAppkey[50];
	char szTranStatusSecret[50];
...
STRUCT_PHQRPAY   strPHQR;

typedef struct
{
	int IMGid;
	BYTE szTransactionTypeName[110];
	BYTE szTranTitle[110];
	BYTE szImageFileName[110];
	
} STRUCT_IMAGE_LIST;

STRUCT_IMAGE_LIST strImageList[100];
