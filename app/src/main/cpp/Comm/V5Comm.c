...
/*BDO: Separate dialing number for BIN Ver -- jzg */
...
int inInitConnectFuncPoint(TRANS_DATA_TABLE *srTransPara)
{
    ...
    else if(srTransPara->usTerminalCommunicationMode == DIAL_UP_MODE)
    {
        ...
        if( strTCT.fShareComEnable != TRUE)
        {
					vdDebug_LogPrintf("**usTerminalCommunicationMode MODEM_MODE START DIAL UP **");
        ...
        else
        {
						...
						vdDebug_LogPrintf("**usTerminalCommunicationMode MODEM_MODE START DIAL UP share comm **");
            srCommFuncPoint.inInitComm = inCTOSS_COMM_Initialize;
            srCommFuncPoint.inGetCommConfig = inGPRS_GetConfigFromTable;
            srCommFuncPoint.inSetCommConfig = inGPRS_SetConnectConfig;
    		    srCommFuncPoint.inCheckComm = inGPRS_CheckComm;
    ...
    else if(srTransPara->usTerminalCommunicationMode == GPRS_MODE)
    {
		...
		else if(srTransPara->usTerminalCommunicationMode == WIFI_MODE)
		{  
		    srCommFuncPoint.inInitComm = inCTOSS_COMM_Initialize;
				srCommFuncPoint.inGetCommConfig = inGPRS_GetConfigFromTable;
				srCommFuncPoint.inSetCommConfig = inGPRS_SetConnectConfig;
...
int inCTOS_inDisconnect(void)
{
    ...
    else
    {
		...
		if(srTransRec.usTerminalCommunicationMode == DIAL_UP_MODE)
		{
            ...
            vdDebug_LogPrintf("DIAL UP N0-inCTOS_inDisconnect result[%d]", result);

    		if(srCommFuncPoint.inEndComm)
    	        result = srCommFuncPoint.inEndComm(&srTransRec);
    		vdDebug_LogPrintf("DIAL UP inEndComm result[%d]", result);
        ...
        else if( strTCT.fShareComEnable != TRUE)
        {
            if(srCommFuncPoint.inDisconnect)
    	        result = srCommFuncPoint.inDisconnect(&srTransRec);
            vdDebug_LogPrintf("DIAL UP SHARE COMM N0-inCTOS_inDisconnect result[%d]", result);

    		if(srCommFuncPoint.inEndComm)
    	        result = srCommFuncPoint.inEndComm(&srTransRec);
    		vdDebug_LogPrintf("DIAL UP SHARE COMM inEndComm result[%d]", result);
...
int inCTOS_CheckInitComm(int inCommType)
{
	...
	vdDebug_LogPrintf("saturn srCommFuncPoint.inInitComm -- START -- [%d]", inCommType);
	...
	else if(inCommType == DIAL_UP_MODE)
	{
		...
		//CTOS_TimeOutSet(TIMER_ID_4, inGetIdleTimeOut(TRUE));
	}	
	else if(inCommType == GPRS_MODE)
	{
        
		vdDebug_LogPrintf("saturn before gprs init comm");
		
		result = srCommFuncPoint.inInitComm(&srTransRec,GPRS_MODE);
		vdDebug_LogPrintf("saturn srCommFuncPoint.inInitComm[%d]", result);
	    //srCommFuncPoint.inGetCommConfig(&srTransRec);
	    //srCommFuncPoint.inSetCommConfig(&srTransRec); 
	    return result;
	...
	vdDebug_LogPrintf("srCommFuncPoint.inInitComm, result = [%d]", result);
#if 0
    if(inCommType == DIAL_UP_MODE || inCommType == ETHERNET_MODE)
    {
        while(1)
        {
            //WaitKey(short Sec)
            vdDebug_LogPrintf("check phone line...");
            CTOS_KBDInKey(&isKey);
            if(isKey)
            {
               vdDebug_LogPrintf("1. check phone line...");
            ...
            //{
            //vdDebug_LogPrintf("2. check phone line...");
            //break;
            //}
            
            if(inCheckConnection() != d_OK)
            {
				if(inCommType == DIAL_UP_MODE)
                    vdDisplayErrorMsgResp2Ex(" ", "Check Phone Line", "Connectivity");
				else
					vdDisplayErrorMsgResp2Ex(" ", "Check LAN","Connectivity");
            }
            else
            {
				CTOS_LCDTClearDisplay();
...
#if 0
		if((strTCT.byTerminalType == 3) || (strTCT.byTerminalType == 4))
		{
			vdDebug_LogPrintf("**usTerminalCommunicationMode ETHERNET_MODE START**");
			srCommFuncPoint.inInitComm = inEthernet_InitEthernet;
			srCommFuncPoint.inGetCommConfig = inEthernet_GetConfigFromTable;
...
//			{
	...
	else if(inCommType == DIAL_UP_MODE)
	{
		...
		//if (0)
		{
			vdDebug_LogPrintf("**usTerminalCommunicationMode MODEM_MODE START  INIT COMM**");
			srCommFuncPoint.inInitComm = inModem_InitModem;
			srCommFuncPoint.inGetCommConfig = inModem_GetConfigFromTable;
			srCommFuncPoint.inSetCommConfig = inModem_SetConnectConfig;
			srCommFuncPoint.inCheckComm = inModem_CheckComm;
	...
	//wifi-mod
		...
		{
			vdDebug_LogPrintf("saturn **usTerminalCommunicationMode WIFI_MODE START**");
		    srCommFuncPoint.inInitComm = inCTOSS_COMM_Initialize;
				srCommFuncPoint.inGetCommConfig = inGPRS_GetConfigFromTable;
				srCommFuncPoint.inSetCommConfig = inGPRS_SetConnectConfig;
				srCommFuncPoint.inCheckComm = inGPRS_CheckComm;
...
int inGPRS_InitGPRS(TRANS_DATA_TABLE *srTransPara,int mode)
	{
		...
		incommGPRSCheckState(d_TCP_IO_PROCESSING,GPRS_TIMEOUT_DISABLE);
			...
			if(bolState != TRUE)
			{
				...
				vdDisplayErrorMsgResp2(" ", " ", "GPRS INIT FAIL");
			CTOS_TCP_GPRSClose();
				CTOS_KBDGet ( &key );
...
int inGPRS_ConnectHost(TRANS_DATA_TABLE *srTransPara)
{
  ...
  BYTE strHostIP[50];
	...
	if(srTransRec.byPackType != DCC_RATEREQUEST && srTransRec.byPackType != DCC_RATEREQUEST_RETRY)
		vdDispTransTitleCardTypeandTotal(srTransRec.byTransType, "Connecting to Pri IP");  /*BDO: Display TOTAL AMOUNT during connection -- sidumili*/
...
int inGPRS_SendData(TRANS_DATA_TABLE *srTransPara,unsigned char *uszSendData,unsigned long ulSendLen)
{
    ...
    for(i=0; i<3; i++)
	{
		#if 1
		if(srTransRec.byPackType != DCC_RATEREQUEST && srTransRec.byPackType != DCC_RATEREQUEST_RETRY)
			vdDispTransTitleCardTypeandTotal(srTransRec.byTransType, "PROCESSING..."); //aaronnino for BDOCLG ver 9.0 fix on issue #00139 HAVE A DEFAULT TITLE DISPLAY OF TXN TYPE CARD LABEL 6 of 8
...
int inModem_InitModem(TRANS_DATA_TABLE *srTransPara,int mode)
{
    ...
    vdDebug_LogPrintf (". shRet OpenErr(%04x)", shRet);     
		
	if (shRet != d_OK){
						vdMyEZLib_LogPrintf(". shRet OpenErr(%04x)", shRet);
						vdMyEZLib_LogPrintf("**inModem_InitModem OpenErr**");

						//Ret = CTOS_ModemOpen(strCPT.inParaMode, strCPT.inHandShake, strCPT.inCountryCode);	/// d_M_MODE_SDLC_FAST
						if (shRet != d_OK){
								vdMyEZLib_LogPrintf(". shRet OpenErr(%04x)", shRet);
								vdMyEZLib_LogPrintf("**inModem_InitModem OpenErr**");
								//CTOS_KBDGet(&bkey); 		 
...
int inModem_CheckComm(TRANS_DATA_TABLE *srTransPara)
{
	...
	if (strCPT.fPreDial == TRUE)
	{
		...
		vdDebug_LogPrintf("inModem_CheckComm - isPredial = %d  shRet=%d",isPredial, shRet);
 	}
...
int inModem_ConnectHost(TRANS_DATA_TABLE *srTransPara)
{
	...
	vdDebug_LogPrintf("inModem_ConnectHost isPredial = %d",isPredial);
	if(srTransRec.byPackType != DCC_RATEREQUEST && srTransRec.byPackType != DCC_RATEREQUEST_RETRY)
	{
		vdDispTransTitleCardTypeandTotal(srTransRec.byTransType, "PROCESSING...");  /*BDO: Display TOTAL AMOUNT during connection -- sidumili*/
...
//1105
	...
	//fix issue 00024, remove DialUp...... msg 20160218
    ...
    {   
...

        {
            ...
            {            
				vdDebug_LogPrintf("Modem d_M_STATUS_MODEM_ONLINE");
				//fix issue 00024, remove Connected msg 20160218
            ...
            else if(dwStatus & d_M_STATUS_NO_DIAL_TONE)
            {
                ...
                //vdDisplayErrorMsg(1, 8, "NO PHONE LINE"); //aaronnino for BDOCLG ver 9.0 fix on issue #0023 Incorrect Error message when terminal has no telephone line
				#if 0
...
int inModem_Disconnection(TRANS_DATA_TABLE *srTransPara)
{
    ...
    vdDebug_LogPrintf("inModem_Disconnection %d", FALSE);
#if 0
    if(isDialUp == FALSE)
    {
...
int inModem_SendData(TRANS_DATA_TABLE *srTransPara,unsigned char *uszSendData,unsigned long ulSendLen)
{
    ...
    while(1)
    {
        if (CTOS_ModemTxReady()== d_OK){
			...
			/*20160229 #issue24, if V3 touch screen terminal, line 14 will still display dial message.
			here just clear the message, can also use function vdCTOS_DispStatusMessage(), 
			replace other CTOS_LCDTPrintXY (1,8,*/
			if ((strTCT.byTerminalType%2) == 0)
			{
				clearLine(14);
			}
       		if(srTransRec.byPackType != DCC_RATEREQUEST && srTransRec.byPackType != DCC_RATEREQUEST_RETRY)
      			vdDispTransTitleCardTypeandTotal(srTransRec.byTransType, "PROCESSING..."); //aaronnino for BDOCLG ver 9.0 fix on issue #00139 HAVE A DEFAULT TITLE DISPLAY OF TXN TYPE CARD LABEL 7 of  8
...
int inModem_CutOffConnection(TRANS_DATA_TABLE *srTransPara)
{
    ...
    vdDebug_LogPrintf("**inModem_CutOffConnection START**");
...
{   
    ...
    vdDebug_LogPrintf("STEP 1 inEthernet_InitEthernet");
...
int inEthernet_GetConfigFromTable(TRANS_DATA_TABLE *srTransPara)
{
    ...
    vdDebug_LogPrintf("STEP 2 inEthernet_GetConfigFromTable");
...
int inEthernet_SetConnectConfig(TRANS_DATA_TABLE *srTransPara)
{
    ...
    vdDebug_LogPrintf("STEP 3 inEthernet_SetConnectConfig");
...
int inEthernet_CheckComm(TRANS_DATA_TABLE *srTransPara)
{
    ...
    vdDebug_LogPrintf("STEP 4 inEthernet_CheckComm");

	vdDebug_LogPrintf("inEthernet_CheckComm");
    ...
    //sockHandle = socket(AF_INET, sockType, IPPROTO_TCP);

	vdDebug_LogPrintf(". srEthernetRec.strRemoteIP =(%s) PORT =(%d)",srEthernetRec.strRemoteIP, srEthernetRec.usRemotePort);
    ...
    vdDebug_LogPrintf("STEP 4 sockethandle = %d  fCommAlreadyOPen = %d", sockHandle, fCommAlreadyOPen);
...
int inEthernet_ConnectHost(TRANS_DATA_TABLE *srTransPara)
{
    ...
    vdDebug_LogPrintf("STEP 5 inEthernet_ConnectHost");


	if (fCommAlreadyOPen == 1)
		return d_OK;
  ...
  srEthernetRec.usRemotePort=0;
   ...
   inHideConnectionDisplay = get_env_int("BDOSM");
	 ...
	 {
	       //vdCTOS_DispStatusMessage("CONNECTING TO PRI IP");
	       vdDispTransTitleCardTypeandTotal(srTransRec.byTransType,"CONNECTING TO PRI IP");
	 }
  ...
  if(srTransRec.byTransType == SETTLE){
	...
	arg = fcntl(sockHandle, F_GETFL, NULL);     
	arg |= O_NONBLOCK;    
	fcntl(sockHandle, F_SETFL, arg);
	
    vdDebug_LogPrintf("Socket handle is: %d", sockHandle);
		errno = 0;

	//TINE:  20JUN2019 - Set timeout for IP connect	
	CTOS_TimeOutSet (TIMER_ID_1 , inGetIdleTimeOut(FALSE));
	vdDebug_LogPrintf("LOOP START");
//LOOP:	
	while(1)
	{
	    shRet = connect(sockHandle, (struct sockaddr*)&sockHost, sizeof(struct sockaddr_in));
			...
			if(CTOS_TimeOutCheck(TIMER_ID_1 )  == d_YES)
	            {
	                ...
	                vdDebug_LogPrintf("LOOP END - TIMEOUT!!!");
	                break;
	            }
			//else
				//goto LOOP;
	...
	//DBGR_MSG("Set to blocking mode again...."); 
	arg = fcntl(sockHandle, F_GETFL, NULL);    
	arg &= (~O_NONBLOCK);    
	fcntl(sockHandle, F_SETFL, arg);
    ...
    vdDebug_LogPrintf("STEP 5 socket connect = %d", shRet);
#endif

	/*BDO: Return this error when NO LAN connected -- sidumili*/
	vdDebug_LogPrintf("::szErrResult[%s]", szErrResult);
#if 0
	if ((atoi(szErrResult) == 113) || (atoi(szErrResult) == 111) || (atoi(szErrResult) == 101))
	{
		CTOS_Delay(500);
...
//check LAN cable if connected		
		...
		if (dwStatus & d_STATUS_ETHERNET_PHYICAL_ONLINE){
			...
			else
			{
				...
				strcat(szDisMsg, "|");
				strcat(szDisMsg, "Check Connectivity");
...
int inEthernet_Disconnection(TRANS_DATA_TABLE *srTransPara)
{
    ...
    vdDebug_LogPrintf("STEP 8 inEthernet_Disconnection");
#ifdef CTOS_API	
   if (1 ==inEthernetOpen)
   	{
...
int inEthernet_SendData(TRANS_DATA_TABLE *srTransPara,unsigned char *uszSendData,unsigned long ulSendLen)
{
    ...
    vdDebug_LogPrintf("STEP 6 inEthernet_SendData");
...
#else
	if(srTransRec.byPackType != DCC_RATEREQUEST && srTransRec.byPackType != DCC_RATEREQUEST_RETRY)
	    vdDispTransTitleCardTypeandTotal(srTransRec.byTransType, "PROCESSING..."); //aaronnino for BDOCLG ver 9.0 fix on issue #00139 HAVE A DEFAULT TITLE DISPLAY OF TXN TYPE CARD LABEL 8 of 8
...
int inEthernet_ReceiveData(TRANS_DATA_TABLE *srTransPara,unsigned char *uszRecData)
{
    ...
    vdDebug_LogPrintf("STEP 7 inEthernet_ReceiveData");

    memset(usTempRecData,0,sizeof(usTempRecData));
    CTOS_TimeOutSet(TIMER_ID_2, 50);

	//TINE:  29MAY2019
	if(srTransRec.byPackType == TC_UPLOAD)
        {
			memset(szDisMsg, 0x00, sizeof(szDisMsg));
			memset(szTitle, 0x00, sizeof(szTitle));
			szGetTransTitle(srTransRec.byTransType, szTitle);
...
*/        
 ...
 //strcat(szDisMsg, "RECEIVING...");
 //usCTOSS_LCDDisplay(szDisMsg); 
 vdDispTransTitleCardTypeandTotal(srTransRec.byTransType, "RECEIVING...");		//TINE: 23MAY2019
            ...
            if(inLen == 0)
            {
                ...
                inLen += (usTempRecData[1] / 16 * 10) + usTempRecData[1] % 16;
						...
						if ((inRealLen == 0) || (inLen == 0)){
							...
							vdDebug_LogPrintf("TEST inCTOSS_GetGPRSSignalEx1 %d", usEthType);
							...
							else
							{
    							...
    							strcpy(szDisMsg, szTitle);
    							strcat(szDisMsg, "|");
    							strcat(szDisMsg, "No host response from bank.");
								usCTOSS_LCDDisplay(szDisMsg);
								CTOS_Beep();
								CTOS_Delay(1500);
...
int inEthernet_CutOffConnection(TRANS_DATA_TABLE *srTransPara)
{
   ...
   vdDebug_LogPrintf("STEP 9 inEthernet_CutOffConnection");

   if (1 ==inEthernetOpen)
   	{
   	    inCTOSS_CheckMemoryStatusEx("before call to CTOS_EthernetClose");
		shRet = CTOS_EthernetClose();
		 inCTOSS_CheckMemoryStatusEx("after call to CTOS_EthernetClose");
		inEthernetOpen= 0;
   	}
...
void vdCTOS_DispStatusMessage(char* szMsg)
{
    ...
    {
        //clearLine(8);// this function is slowly
		CTOS_LCDTPrintXY(1, 8, "                                        ");
        CTOS_LCDTPrintXY(7, 8, szMsg);
    }
}
void vdCTOS_DispStatusMessageEx(char* szMsg)
{
if ((strTCT.byTerminalType%2) == 0){
//clearLine(13);// this function is slowly
CTOS_LCDTPrintXY(1, 13, "                                        ");
CTOS_LCDTPrintXY(1, 13, szMsg);
}
else{
//clearLine(7);// this function is slowly
CTOS_LCDTPrintXY(1, 7, "                                        ");
...
int inCheckConnection(void) /*currently for IP only*/
{
    ...
    if(inCheckEthernetSocketConnected() == BROKEN_PIPE)
        return ST_COMMS_DISCONNECT;
    else if(inCheckEthernetConnected() == ST_COMMS_DISCONNECT)
        return ST_COMMS_DISCONNECT;
	else
		return d_OK;
}

int inCheckModemConnected(void)
{
    DWORD dwStatus;
    char szTempModemStatusRet[8+1];
    ...
    vdDebug_LogPrintf("inCheckModemConnected - dwStatus[%08X]", dwStatus);
    memset(szTempModemStatusRet,0x00,sizeof(szTempModemStatusRet));
    sprintf(szTempModemStatusRet,"%08X",dwStatus);
    
    if(memcmp(szTempModemStatusRet, "00000804", 8)==0)
        return d_NO;
