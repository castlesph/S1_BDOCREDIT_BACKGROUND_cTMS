...
int inCTOS_LogonFlowProcess(void)
{
	...
	vdDebug_LogPrintf("POSLogon, inCTOS_PreConnect");
    inRet = inCTOS_PreConnect();
    if(d_OK != inRet)
        return inRet;

	vdDebug_LogPrintf("POSLogon, inCTOS_CheckIssuerEnable");
	...
	vdDebug_LogPrintf("POSLogon, inBuildAndSendIsoData = [%d]", inRet);
    if(d_OK != inRet){
	...
	vdDebug_LogPrintf("LOGON SUCCESSFUL");
	//sidumili
...
int inCTOS_LOGON(void)
{
	...
	vdDebug_LogPrintf("POSLogon, inCTOS_LogonFlowProcess = [%d]", inRet);
    inCTOS_inDisconnect();
