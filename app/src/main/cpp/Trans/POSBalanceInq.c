...
int inCTOS_BalanceInquiryFlowProcess(void)
{
    ...
    //load IIT to fix crash if atpBinroute is enable
    inIITRead(strCDT.IITid);

	vdDebug_LogPrintf("saturn load IIT %s %d %d", strIIT.szIssuerLogo, strIIT.inIssuerNumber, strCDT.IITid);
	...
	vdDebug_LogPrintf("saturn after select host");

    if (inMultiAP_CheckMainAPStatus() == d_OK)
    {
       
	   vdDebug_LogPrintf("saturn bal inquiry main app");
    ...
    else
    {
	 ...
	 vdDebug_LogPrintf("saturn bal inquiry check sub-app");
        if (inMultiAP_CheckSubAPStatus() == d_OK)
        {
           
		   vdDebug_LogPrintf("saturn bal inquiry sub-app");
