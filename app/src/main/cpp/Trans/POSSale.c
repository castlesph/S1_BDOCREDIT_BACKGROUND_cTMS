...
int inGetPolicyNumber(void)
{
    ...
    vdDebug_LogPrintf("inGetPolicyNumber");
    ...
    vdDebug_LogPrintf("1. inGetPolicyNumber");
    ...
    vdDebug_LogPrintf("2. inGetPolicyNumber");
	...
	vdDebug_LogPrintf("usMaxLen = [%d]", usMaxLen);
	...
	while(1)
	{
        ...
        //issue-00256
		//usRet = InputStringAlpha2(1, usInputLine, 0x00, 0x02, baBuf, &usMaxLen, usMinLen, inGetIdleTimeOut(FALSE));
		//TINE:  use InputAlphaUI to provide android keypad screen 
		usRet = InputAlphaUI(0x00,  0x02, baBuf, usMaxLen, usMinLen, inGetIdleTimeOut(FALSE), szDisplay);
