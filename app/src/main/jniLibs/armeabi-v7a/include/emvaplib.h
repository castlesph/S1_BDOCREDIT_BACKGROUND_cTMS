...
#define d_EMVAPLIB_ERR_LASTERR_MISSING_TERMINAL_DATA                    0x0075
...
#define d_EMVAPLIB_ERR_VIRTUAL_PIN_NOT_PROCESSING                       0x0111
...
#define d_EMVAPLIB_ERR_CAS_GAC2_WRONG_CID                               0x1082
...
typedef struct
{
	...
	//Add in Version 2
	IN BYTE bIsKSNFixed; //To set Enc use same KSN or Not, and affect the MSREncFlow behavior when bMSRFlow = 1~3 (In the First time Enc)
	//MSREncFlow 0 : Customize by user (To check the input array baMSREncOrder & baMSRCuerntKey)
	//MSREncFlow 1 : T1 Enc, T2 Enc, T3 Enc, T2 MAC, T3 MAC, T3 MAC  (All KSN fixed in a swipe, First time generate a new KSN(when T1 Enc) when bIsKSNFixed == 0 )
	//MSREncFlow 2 : T1 Enc, T1 MAC, T2 Enc, T2 MAC, T3 Enc, T3 MAC  (KSN ++ for each process, First time generate a new KSN(when T1 Enc) when bIsKSNFixed == 0 )
	//MSREncFlow 3 : T1 Enc, T1 MAC, T2 Enc, T2 MAC, T3 Enc, T3 MAC  (KSN fixed for each Track, differnt Track KSN++, First time generate a new KSN(when T1 Enc) when bIsKSNFixed == 0 )
	IN BYTE bMSREncFlow;			
	//IN BYTE baMSREncOrder[6];	//3 Enc + 3 MAC
	//IN BYTE baMSRKSNFixed[6];	//3 Enc + 3 MAC	
...
USHORT EMV_SetConfiguration(IN char* pEMVConfigFileName);
USHORT EMV_SetConfigurationFile(IN char* pEMVConfigFileName);
USHORT EMV_DelConfigurationFileAll(void);
USHORT EMV_DelConfigurationFile(IN char* pEMVConfigFileName);
USHORT EMV_VerifyConfiguration(IN char* pEMVConfigFileName, IN BYTE bLoadWhenSuccess);
...
USHORT EMV_TxnDataSetEx(IN BYTE *baTLVBuf, IN USHORT usTLVLen);
//Pre setting before txn, the tags will be add to kernel before GPO
USHORT EMV_TxnPreDataSet(IN BYTE *baTLVBuf, IN USHORT usTLVLen);
...
USHORT EMV_PartialTransactionSet(ULONG ulType);
USHORT EMV_PartialTransactionGet(ULONG *pulType);

USHORT EMV_GetLastError(void);

USHORT EMV_NullPINEnable(void);
USHORT EMV_NullPINDisable(void);
USHORT EMV_VirtualPINCancel(void);
USHORT EMV_PINClearButtonBehaviorSet(BYTE bBehavior);
USHORT EMV_PINClearButtonBehaviorGet(BYTE *pbBehavior);
...
#define d_EVENTID_SELECT_MULTI_LANGUAGE								18
#define d_EVENTID_SHOW_ADVICE										19
#define d_EVENTID_TXN_FORCED_ACCEPTANCE								20
#define d_EVENTID_OFFLINE_ONLY_TERMINAL_DEFAULT_ACTION_CODE			21
#define d_EVENTID_ONLINE_ONLY_TERMINAL_DEFAULT_ACTION_CODE			22
#define d_EVENTID_APPLICATION_SELECTION_METHOD						30
#define d_EVENTID_APP_LIST_ALWAYS									31

#define d_EVENTID_OFFLINE_PIN_VERIFY_RESULT							0xC0
#define d_EVENTID_DEBUG_MSG											0xC1
#define d_EVENTID_PIN_BYPASS										0xC2
#define d_EVENTID_CANCEL_PIN_CONFIRM								0xC3
...
//variable for d_EVENTID_SELECT_MULTI_LANGUAGE
typedef void (*EVENT_SELECT_MULTI_LANGUAGE)(IN BYTE *pbLanguagePreference, IN BYTE bLanguagePreferenceLen);

//variable for d_EVENTID_SHOW_ADVICE
typedef USHORT (*EVENT_SHOW_ADVICE)(IN BYTE bAdvice);

//variable for d_EVENTID_TXN_FORCED_ACCEPTANCE
typedef USHORT (*EVENT_TXN_FORCED_ACCEPTANCE)(OUT BYTE *pbForcedAccept);
//pbForcedAccept : 
// 0x01 : Txn Forced Accept

//variable for d_EVENTID_OFFLINE_ONLY_TERMINAL_DEFAULT_ACTION_CODE
typedef void (*EVENT_OFFLINE_ONLY_TERMINAL_DEFAULT_ACTION_CODE)(OUT BYTE *pbOption);
//pbOption
// 0x00 : Prior to 1st Gen AC
// 0x01 : After 1st Gen AC

//variable for d_EVENTID_ONLINE_ONLY_TERMINAL_DEFAULT_ACTION_CODE
typedef void (*EVENT_ONLINE_ONLY_TERMINAL_DEFAULT_ACTION_CODE)(OUT BYTE *pbOption);
//pbOption
// 0x00 : Normal
// 0x01 : Skip
...
//variable for d_EVENTID_DEBUG_MSG
typedef void (*EVENT_DEBUG_MSG)(IN BYTE *baMsg, IN USHORT usMsgLen);

//variable for d_EVENTID_PIN_BYPASS
typedef USHORT (*EVENT_PIN_BYPASS)(OUT BYTE *pbAction);
//pbAction
//0x00 - Bypass PIN
//0x01 - Don't bypass PIN, back to Get PIN Process
//0x02 - Don't bypass PIN, trigger OnShowVirtualPIN() again then back to Get PIN Process

//variable for d_EVENTID_CANCEL_PIN_CONFIRM
typedef USHORT (*EVENT_CANCEL_PIN_CONFIRM)(IN BYTE bStatus, OUT BYTE *pbAction);
//bStatus
//0x00 - Inform
//0x01 - Poliing check
