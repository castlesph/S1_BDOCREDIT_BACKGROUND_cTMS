...
public class VPINPad
	{
		...
		private int [][] PINPadLocation;
		private TextView[] textView;
		boolean isViewCreated = false;

		public VPINPad(final Activity activity, final RelativeLayout layout)
		{
			PINPadLocation = new int[16][5];
				...
				public void run() {
					...
					for(int i = 0;i < 5;i ++)
					{
						...
						for(int j = 0;j < 3;j ++)
						{
							...
							{
								case 0:
									textView[index].setText(new String(new byte [] {PINPadBody[count++]}));
				...
				public void onGlobalLayout()
				{
					...
					synchronized(PINPadLocation)
					{
						for (int i = 0; i < textView.length; i++)
						{
							...
							PINPadLocation[i][0] = x;
							PINPadLocation[i][1] = y;
							PINPadLocation[i][2] = x + textView[i].getWidth();
							PINPadLocation[i][3] = y + textView[i].getHeight();
